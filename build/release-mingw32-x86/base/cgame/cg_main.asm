export vmMain
code
proc vmMain 20 12
ADDRLP4 0
ADDRFP4 0
INDIRI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 0
LTI4 $118
ADDRLP4 0
INDIRI4
CNSTI4 10
GTI4 $118
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 $132
ADDP4
INDIRP4
JUMPV
lit
align 4
LABELV $132
address $120
address $121
address $122
address $124
address $125
address $126
address $127
address $128
address $129
address $123
address $130
code
LABELV $120
ADDRFP4 4
INDIRI4
ARGI4
ADDRFP4 8
INDIRI4
ARGI4
ADDRFP4 12
INDIRI4
ARGI4
ADDRGP4 CG_Init
CALLV
pop
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $121
ADDRGP4 CG_Shutdown
CALLV
pop
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $122
ADDRLP4 4
ADDRGP4 CG_ConsoleCommand
CALLI4
ASGNI4
ADDRLP4 4
INDIRI4
RETI4
ADDRGP4 $117
JUMPV
LABELV $123
ADDRGP4 CG_AddNotifyText
CALLV
pop
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $124
ADDRFP4 4
INDIRI4
ARGI4
ADDRFP4 8
INDIRI4
ARGI4
ADDRFP4 12
INDIRI4
ARGI4
ADDRGP4 CG_DrawActiveFrame
CALLV
pop
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $125
ADDRLP4 8
ADDRGP4 CG_CrosshairPlayer
CALLI4
ASGNI4
ADDRLP4 8
INDIRI4
RETI4
ADDRGP4 $117
JUMPV
LABELV $126
ADDRLP4 12
ADDRGP4 CG_LastAttacker
CALLI4
ASGNI4
ADDRLP4 12
INDIRI4
RETI4
ADDRGP4 $117
JUMPV
LABELV $127
ADDRFP4 4
INDIRI4
ARGI4
ADDRFP4 8
INDIRI4
ARGI4
ADDRGP4 CG_KeyEvent
CALLV
pop
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $128
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $129
ADDRFP4 4
INDIRI4
ARGI4
ADDRGP4 CG_EventHandling
CALLV
pop
CNSTI4 0
RETI4
ADDRGP4 $117
JUMPV
LABELV $130
ADDRLP4 16
ADDRGP4 CG_VoIPString
CALLP4
ASGNP4
ADDRLP4 16
INDIRP4
CVPU4 4
CVUI4 4
RETI4
ADDRGP4 $117
JUMPV
LABELV $118
ADDRGP4 $131
ARGP4
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 CG_Error
CALLV
pop
LABELV $119
CNSTI4 -1
RETI4
LABELV $117
endproc vmMain 20 12
data
align 4
LABELV cvarTable
address cg_drawGun
address $134
address $135
byte 4 1
address cg_viewsize
address $136
address $137
byte 4 1
address cg_stereoSeparation
address $138
address $139
byte 4 1
address cg_shadows
address $140
address $135
byte 4 1
address cg_draw2D
address $141
address $135
byte 4 1
address cg_drawTimer
address $142
address $135
byte 4 1
address cg_drawClock
address $143
address $144
byte 4 1
address cg_drawFPS
address $145
address $135
byte 4 1
address cg_drawDemoState
address $146
address $135
byte 4 1
address cg_drawSnapshot
address $147
address $144
byte 4 1
address cg_drawChargeBar
address $148
address $135
byte 4 1
address cg_drawCrosshair
address $149
address $150
byte 4 1
address cg_drawCrosshairNames
address $151
address $135
byte 4 1
address cg_crosshairSize
address $152
address $135
byte 4 1
address cg_addMarks
address $153
address $135
byte 4 1
address cg_lagometer
address $154
address $144
byte 4 1
address cg_drawSpeed
address $155
address $144
byte 4 1
address cg_teslaTrailTime
address $156
address $157
byte 4 1
address cg_gun_x
address $158
address $144
byte 4 512
address cg_gun_y
address $159
address $144
byte 4 512
address cg_gun_z
address $160
address $144
byte 4 512
address cg_centertime
address $161
address $162
byte 4 512
address cg_runpitch
address $163
address $164
byte 4 1
address cg_runroll
address $165
address $166
byte 4 1
address cg_swingSpeed
address $167
address $168
byte 4 512
address cg_animSpeed
address $169
address $135
byte 4 512
address cg_debugAnim
address $170
address $144
byte 4 512
address cg_debugPosition
address $171
address $144
byte 4 512
address cg_debugEvents
address $172
address $144
byte 4 512
address cg_errorDecay
address $173
address $137
byte 4 0
address cg_nopredict
address $174
address $144
byte 4 0
address cg_debugMove
address $175
address $144
byte 4 0
address cg_noPlayerAnims
address $176
address $144
byte 4 512
address cg_showmiss
address $177
address $144
byte 4 0
address cg_footsteps
address $178
address $135
byte 4 512
address cg_tracerChance
address $179
address $139
byte 4 512
address cg_tracerWidth
address $180
address $135
byte 4 512
address cg_tracerLength
address $181
address $137
byte 4 512
address cg_thirdPersonRange
address $182
address $183
byte 4 1
address cg_thirdPerson
address $184
address $144
byte 4 512
address cg_thirdPersonAngle
address $185
address $144
byte 4 512
address cg_thirdPersonPitchFollow
address $186
address $144
byte 4 0
address cg_thirdPersonShoulderViewMode
address $187
address $135
byte 4 1
address cg_stats
address $188
address $144
byte 4 0
address cg_drawTeamOverlay
address $189
address $135
byte 4 1
address cg_teamOverlayMaxPlayers
address $190
address $191
byte 4 1
address cg_teamOverlayUserinfo
address $192
address $135
byte 4 3
address cg_teamChatsOnly
address $193
address $144
byte 4 1
address cg_noPrintDuplicate
address $194
address $144
byte 4 1
address cg_noVoiceChats
address $195
address $144
byte 4 1
address cg_noVoiceText
address $196
address $144
byte 4 1
address cg_drawSurfNormal
address $197
address $144
byte 4 512
address cg_drawBBOX
address $198
address $144
byte 4 512
address cg_wwSmoothTime
address $199
address $200
byte 4 1
byte 4 0
address $201
address $135
byte 4 3
byte 4 0
address $202
address $135
byte 4 3
byte 4 0
address $203
address $144
byte 4 3
address cg_stickySpec
address $204
address $135
byte 4 3
address cg_sprintToggle
address $205
address $144
byte 4 3
address cg_unlagged
address $206
address $135
byte 4 3
byte 4 0
address $207
address $208
byte 4 3
address cg_depthSortParticles
address $209
address $135
byte 4 1
address cg_bounceParticles
address $210
address $144
byte 4 1
address cg_consoleLatency
address $211
address $212
byte 4 1
address cg_lightFlare
address $213
address $162
byte 4 1
address cg_debugParticles
address $214
address $144
byte 4 512
address cg_debugTrails
address $215
address $144
byte 4 512
address cg_debugPVS
address $216
address $144
byte 4 512
address cg_disableWarningDialogs
address $217
address $144
byte 4 1
address cg_disableUpgradeDialogs
address $218
address $144
byte 4 1
address cg_disableBuildDialogs
address $219
address $144
byte 4 1
address cg_disableCommandDialogs
address $220
address $144
byte 4 1
address cg_disableScannerPlane
address $221
address $144
byte 4 1
address cg_tutorial
address $222
address $135
byte 4 1
address cg_hudFiles
address $223
address $224
byte 4 1
byte 4 0
address $225
address $226
byte 4 1
byte 4 0
address $227
address $226
byte 4 1
byte 4 0
address $228
address $226
byte 4 1
address cg_painBlendUpRate
address $229
address $230
byte 4 0
address cg_painBlendDownRate
address $231
address $232
byte 4 0
address cg_painBlendMax
address $233
address $234
byte 4 0
address cg_painBlendScale
address $235
address $236
byte 4 0
address cg_painBlendZoom
address $237
address $238
byte 4 0
address cg_debugVoices
address $239
address $144
byte 4 0
address ui_currentClass
address $240
address $144
byte 4 64
address ui_carriage
address $241
address $226
byte 4 64
address ui_stages
address $242
address $243
byte 4 64
address ui_dialog
address $244
address $245
byte 4 64
address ui_voteActive
address $246
address $144
byte 4 64
address ui_humanTeamVoteActive
address $247
address $144
byte 4 64
address ui_alienTeamVoteActive
address $248
address $144
byte 4 64
address cg_debugRandom
address $249
address $144
byte 4 0
address cg_optimizePrediction
address $250
address $135
byte 4 1
address cg_projectileNudge
address $251
address $135
byte 4 1
address cg_paused
address $252
address $144
byte 4 64
address cg_blood
address $253
address $135
byte 4 1
address cg_synchronousClients
address $254
address $144
byte 4 0
address cg_timescaleFadeEnd
address $255
address $135
byte 4 512
address cg_timescaleFadeSpeed
address $256
address $144
byte 4 512
address cg_timescale
address $257
address $135
byte 4 0
address cg_smoothClients
address $258
address $144
byte 4 3
address cg_cameraMode
address $259
address $144
byte 4 512
address pmove_fixed
address $260
address $144
byte 4 0
address pmove_msec
address $261
address $191
byte 4 0
address cg_noTaunt
address $262
address $144
byte 4 1
address cg_voice
address $263
address $264
byte 4 3
address cg_emoticons
address $265
address $135
byte 4 33
address cg_chatTeamPrefix
address $266
address $135
byte 4 1
address cg_translateCenterPrint
address $267
address $144
byte 4 1
align 4
LABELV cvarTableSize
byte 4 109
export CG_RegisterCvars
code
proc CG_RegisterCvars 1036 16
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRLP4 0
ADDRGP4 cvarTable
ASGNP4
ADDRGP4 $272
JUMPV
LABELV $269
ADDRLP4 0
INDIRP4
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
CNSTI4 4
ADDP4
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
CNSTI4 8
ADDP4
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
CNSTI4 12
ADDP4
INDIRI4
ARGI4
ADDRGP4 trap_Cvar_Register
CALLV
pop
LABELV $270
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
ADDRLP4 0
INDIRP4
CNSTI4 16
ADDP4
ASGNP4
LABELV $272
ADDRLP4 4
INDIRI4
ADDRGP4 cvarTableSize
INDIRI4
LTI4 $269
ADDRGP4 $273
ARGP4
ADDRLP4 8
ARGP4
CNSTI4 1024
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 8
ARGP4
ADDRLP4 1032
ADDRGP4 atoi
CALLI4
ASGNI4
ADDRGP4 cgs+31464
ADDRLP4 1032
INDIRI4
ASGNI4
LABELV $268
endproc CG_RegisterCvars 1036 16
proc CG_SetUIVars 1040 12
ADDRGP4 cg+36
INDIRP4
CVPU4 4
CNSTU4 0
NEU4 $276
ADDRGP4 $275
JUMPV
LABELV $276
ADDRLP4 4
CNSTI1 0
ASGNI1
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $279
ADDRLP4 0
INDIRI4
ARGI4
ADDRGP4 cg+36
INDIRP4
CNSTI4 236
ADDP4
ARGP4
ADDRLP4 1028
ADDRGP4 BG_InventoryContainsWeapon
CALLI4
ASGNI4
ADDRLP4 1028
INDIRI4
CNSTI4 0
EQI4 $283
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 1032
ADDRGP4 BG_Weapon
CALLP4
ASGNP4
ADDRLP4 1032
INDIRP4
CNSTI4 80
ADDP4
INDIRI4
CNSTI4 0
EQI4 $283
ADDRGP4 $286
ARGP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 1036
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 4
ARGP4
ADDRLP4 1036
INDIRP4
ARGP4
ADDRGP4 strcat
CALLP4
pop
LABELV $283
LABELV $280
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 27
LTI4 $279
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $287
ADDRLP4 0
INDIRI4
ARGI4
ADDRGP4 cg+36
INDIRP4
CNSTI4 236
ADDP4
ARGP4
ADDRLP4 1028
ADDRGP4 BG_InventoryContainsUpgrade
CALLI4
ASGNI4
ADDRLP4 1028
INDIRI4
CNSTI4 0
EQI4 $291
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 1032
ADDRGP4 BG_Upgrade
CALLP4
ASGNP4
ADDRLP4 1032
INDIRP4
CNSTI4 32
ADDP4
INDIRI4
CNSTI4 0
EQI4 $291
ADDRGP4 $294
ARGP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 1036
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 4
ARGP4
ADDRLP4 1036
INDIRP4
ARGP4
ADDRGP4 strcat
CALLP4
pop
LABELV $291
LABELV $288
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 9
LTI4 $287
ADDRLP4 4
ARGP4
ADDRGP4 $295
ARGP4
ADDRGP4 strcat
CALLP4
pop
ADDRGP4 $241
ARGP4
ADDRLP4 4
ARGP4
ADDRGP4 trap_Cvar_Set
CALLV
pop
ADDRGP4 $296
ARGP4
ADDRGP4 cgs+34776
INDIRI4
ARGI4
ADDRGP4 cgs+34780
INDIRI4
ARGI4
ADDRLP4 1028
ADDRGP4 va
CALLP4
ASGNP4
ADDRGP4 $242
ARGP4
ADDRLP4 1028
INDIRP4
ARGP4
ADDRGP4 trap_Cvar_Set
CALLV
pop
LABELV $275
endproc CG_SetUIVars 1040 12
export CG_UpdateCvars
proc CG_UpdateCvars 8 4
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRLP4 0
ADDRGP4 cvarTable
ASGNP4
ADDRGP4 $303
JUMPV
LABELV $300
ADDRLP4 0
INDIRP4
INDIRP4
CVPU4 4
CNSTU4 0
EQU4 $304
ADDRLP4 0
INDIRP4
INDIRP4
ARGP4
ADDRGP4 trap_Cvar_Update
CALLV
pop
LABELV $304
LABELV $301
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
ADDRLP4 0
INDIRP4
CNSTI4 16
ADDP4
ASGNP4
LABELV $303
ADDRLP4 4
INDIRI4
ADDRGP4 cvarTableSize
INDIRI4
LTI4 $300
ADDRGP4 CG_SetUIVars
CALLV
pop
LABELV $299
endproc CG_UpdateCvars 8 4
export CG_CrosshairPlayer
proc CG_CrosshairPlayer 0 0
ADDRGP4 cg+109556
INDIRI4
ADDRGP4 cg+117432
INDIRI4
CNSTI4 1000
ADDI4
LEI4 $307
CNSTI4 -1
RETI4
ADDRGP4 $306
JUMPV
LABELV $307
ADDRGP4 cg+117428
INDIRI4
RETI4
LABELV $306
endproc CG_CrosshairPlayer 0 0
export CG_LastAttacker
proc CG_LastAttacker 0 0
ADDRGP4 cg+117444
INDIRI4
CNSTI4 0
NEI4 $313
CNSTI4 -1
RETI4
ADDRGP4 $312
JUMPV
LABELV $313
ADDRGP4 cg+36
INDIRP4
CNSTI4 320
ADDP4
INDIRI4
RETI4
LABELV $312
endproc CG_LastAttacker 0 0
export CG_RemoveNotifyLine
proc CG_RemoveNotifyLine 28 4
ADDRGP4 cg+127524
INDIRI4
CNSTI4 0
NEI4 $318
ADDRGP4 $317
JUMPV
LABELV $318
ADDRLP4 4
ADDRGP4 cg+127268+4
INDIRI4
ASGNI4
ADDRGP4 cg+119076
ARGP4
ADDRLP4 12
ADDRGP4 strlen
CALLI4
ASGNI4
ADDRLP4 8
ADDRLP4 12
INDIRI4
ADDRLP4 4
INDIRI4
SUBI4
ASGNI4
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $327
JUMPV
LABELV $324
ADDRLP4 0
INDIRI4
ADDRGP4 cg+119076
ADDP4
ADDRLP4 0
INDIRI4
ADDRLP4 4
INDIRI4
ADDI4
ADDRGP4 cg+119076
ADDP4
INDIRI1
ASGNI1
LABELV $325
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $327
ADDRLP4 0
INDIRI4
ADDRLP4 8
INDIRI4
LEI4 $324
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $333
JUMPV
LABELV $330
ADDRLP4 20
ADDRLP4 0
INDIRI4
CNSTI4 3
LSHI4
ASGNI4
ADDRLP4 20
INDIRI4
ADDRGP4 cg+127268
ADDP4
ADDRLP4 20
INDIRI4
ADDRGP4 cg+127268+8
ADDP4
INDIRB
ASGNB 8
LABELV $331
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $333
ADDRLP4 0
INDIRI4
ADDRGP4 cg+127524
INDIRI4
LTI4 $330
ADDRLP4 24
ADDRGP4 cg+127524
ASGNP4
ADDRLP4 24
INDIRP4
ADDRLP4 24
INDIRP4
INDIRI4
CNSTI4 1
SUBI4
ASGNI4
LABELV $317
endproc CG_RemoveNotifyLine 28 4
export CG_AddNotifyText
proc CG_AddNotifyText 8224 12
ADDRLP4 0
ARGP4
CNSTI4 8192
ARGI4
ADDRGP4 trap_LiteralArgs
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $340
ADDRGP4 cg+119076
CNSTI1 0
ASGNI1
ADDRGP4 cg+127524
CNSTI4 0
ASGNI4
ADDRGP4 $339
JUMPV
LABELV $340
ADDRLP4 0
ARGP4
ADDRLP4 8200
ADDRGP4 strlen
CALLI4
ASGNI4
ADDRLP4 8192
ADDRLP4 8200
INDIRI4
ASGNI4
ADDRGP4 cg+119076
ARGP4
ADDRLP4 8204
ADDRGP4 strlen
CALLI4
ASGNI4
ADDRLP4 8196
ADDRLP4 8204
INDIRI4
ASGNI4
ADDRGP4 cg_noPrintDuplicate+12
INDIRI4
CNSTI4 0
EQI4 $345
ADDRLP4 8196
INDIRI4
ADDRLP4 8192
INDIRI4
LTI4 $345
ADDRLP4 8196
INDIRI4
ADDRGP4 cg+119076
ADDP4
ADDRLP4 8192
INDIRI4
SUBP4
ARGP4
ADDRLP4 0
ARGP4
ADDRLP4 8216
ADDRGP4 strcmp
CALLI4
ASGNI4
ADDRLP4 8216
INDIRI4
CNSTI4 0
NEI4 $345
ADDRGP4 $339
JUMPV
LABELV $345
ADDRGP4 cg+127524
INDIRI4
CNSTI4 32
NEI4 $349
ADDRGP4 CG_RemoveNotifyLine
CALLV
pop
LABELV $349
ADDRGP4 cg+119076
ARGP4
CNSTI4 8192
ARGI4
ADDRLP4 0
ARGP4
ADDRGP4 Q_strcat
CALLV
pop
ADDRGP4 cg+127524
INDIRI4
CNSTI4 3
LSHI4
ADDRGP4 cg+127268
ADDP4
ADDRGP4 cg+109556
INDIRI4
ASGNI4
ADDRGP4 cg+127524
INDIRI4
CNSTI4 3
LSHI4
ADDRGP4 cg+127268+4
ADDP4
ADDRLP4 8192
INDIRI4
ASGNI4
ADDRLP4 8220
ADDRGP4 cg+127524
ASGNP4
ADDRLP4 8220
INDIRP4
ADDRLP4 8220
INDIRP4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $339
endproc CG_AddNotifyText 8224 12
export CG_Printf
proc CG_Printf 1028 16
ADDRLP4 0
ADDRFP4 0+4
ASGNP4
ADDRLP4 4
ARGP4
CNSTU4 1024
ARGU4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 Q_vsnprintf
CALLI4
pop
ADDRLP4 0
CNSTP4 0
ASGNP4
ADDRLP4 4
ARGP4
ADDRGP4 trap_Print
CALLV
pop
LABELV $360
endproc CG_Printf 1028 16
export CG_Error
proc CG_Error 1028 16
ADDRLP4 0
ADDRFP4 0+4
ASGNP4
ADDRLP4 4
ARGP4
CNSTU4 1024
ARGU4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 Q_vsnprintf
CALLI4
pop
ADDRLP4 0
CNSTP4 0
ASGNP4
ADDRLP4 4
ARGP4
ADDRGP4 trap_Error
CALLV
pop
LABELV $362
endproc CG_Error 1028 16
export Com_Error
proc Com_Error 1028 16
ADDRLP4 0
ADDRFP4 4+4
ASGNP4
ADDRLP4 4
ARGP4
CNSTU4 1024
ARGU4
ADDRFP4 4
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 Q_vsnprintf
CALLI4
pop
ADDRLP4 0
CNSTP4 0
ASGNP4
ADDRGP4 $366
ARGP4
ADDRLP4 4
ARGP4
ADDRGP4 CG_Error
CALLV
pop
LABELV $364
endproc Com_Error 1028 16
export Com_Printf
proc Com_Printf 1028 16
ADDRLP4 0
ADDRFP4 0+4
ASGNP4
ADDRLP4 4
ARGP4
CNSTU4 1024
ARGU4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 Q_vsnprintf
CALLI4
pop
ADDRLP4 0
CNSTP4 0
ASGNP4
ADDRGP4 $366
ARGP4
ADDRLP4 4
ARGP4
ADDRGP4 CG_Printf
CALLV
pop
LABELV $367
endproc Com_Printf 1028 16
bss
align 1
LABELV $370
skip 1024
export CG_Argv
code
proc CG_Argv 0 12
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 $370
ARGP4
CNSTI4 1024
ARGI4
ADDRGP4 trap_Argv
CALLV
pop
ADDRGP4 $370
RETP4
LABELV $369
endproc CG_Argv 0 12
export CG_FileExists
proc CG_FileExists 4 12
ADDRFP4 0
INDIRP4
ARGP4
CNSTP4 0
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 0
ADDRGP4 trap_FS_FOpenFile
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
RETI4
LABELV $371
endproc CG_FileExists 4 12
proc CG_RegisterSounds 232 16
ADDRGP4 $375
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 72
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+468
ADDRLP4 72
INDIRI4
ASGNI4
ADDRGP4 $378
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 76
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+472
ADDRLP4 76
INDIRI4
ASGNI4
ADDRGP4 $381
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 80
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+476
ADDRLP4 80
INDIRI4
ASGNI4
ADDRGP4 $384
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 84
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+480
ADDRLP4 84
INDIRI4
ASGNI4
ADDRGP4 $387
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 88
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+484
ADDRLP4 88
INDIRI4
ASGNI4
ADDRGP4 $390
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 92
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+524
ADDRLP4 92
INDIRI4
ASGNI4
ADDRGP4 $393
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 96
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+528
ADDRLP4 96
INDIRI4
ASGNI4
ADDRGP4 $396
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 100
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+532
ADDRLP4 100
INDIRI4
ASGNI4
ADDRGP4 $399
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 104
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+276
ADDRLP4 104
INDIRI4
ASGNI4
ADDRGP4 $402
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 108
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+284
ADDRLP4 108
INDIRI4
ASGNI4
ADDRGP4 $405
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 112
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+404
ADDRLP4 112
INDIRI4
ASGNI4
ADDRGP4 $408
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 116
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+280
ADDRLP4 116
INDIRI4
ASGNI4
ADDRGP4 $411
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 120
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+384
ADDRLP4 120
INDIRI4
ASGNI4
ADDRGP4 $414
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 124
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+388
ADDRLP4 124
INDIRI4
ASGNI4
ADDRGP4 $417
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 128
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+392
ADDRLP4 128
INDIRI4
ASGNI4
ADDRGP4 $420
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 132
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+396
ADDRLP4 132
INDIRI4
ASGNI4
ADDRGP4 $423
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 136
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+428
ADDRLP4 136
INDIRI4
ASGNI4
ADDRGP4 $426
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 140
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+432
ADDRLP4 140
INDIRI4
ASGNI4
ADDRGP4 $429
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 144
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+436
ADDRLP4 144
INDIRI4
ASGNI4
ADDRGP4 $432
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 148
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+272
ADDRLP4 148
INDIRI4
ASGNI4
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $433
ADDRLP4 4
ARGP4
CNSTI4 64
ARGI4
ADDRGP4 $437
ARGP4
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRLP4 4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 152
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+234996+288
ADDP4
ADDRLP4 152
INDIRI4
ASGNI4
ADDRLP4 4
ARGP4
CNSTI4 64
ARGI4
ADDRGP4 $440
ARGP4
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRLP4 4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 156
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+234996+288+16
ADDP4
ADDRLP4 156
INDIRI4
ASGNI4
ADDRLP4 4
ARGP4
CNSTI4 64
ARGI4
ADDRGP4 $444
ARGP4
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRLP4 4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 160
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+234996+288+48
ADDP4
ADDRLP4 160
INDIRI4
ASGNI4
ADDRLP4 4
ARGP4
CNSTI4 64
ARGI4
ADDRGP4 $448
ARGP4
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRLP4 4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 164
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+234996+288+32
ADDP4
ADDRLP4 164
INDIRI4
ASGNI4
LABELV $434
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 4
LTI4 $433
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $452
ADDRLP4 0
INDIRI4
CNSTI4 286
ADDI4
ARGI4
ADDRLP4 152
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 68
ADDRLP4 152
INDIRP4
ASGNP4
ADDRLP4 68
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $456
ADDRGP4 $454
JUMPV
LABELV $456
ADDRLP4 68
INDIRP4
INDIRI1
CVII4 1
CNSTI4 42
NEI4 $458
ADDRGP4 $453
JUMPV
LABELV $458
ADDRLP4 68
INDIRP4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 156
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+36336
ADDP4
ADDRLP4 156
INDIRI4
ASGNI4
LABELV $453
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 256
LTI4 $452
LABELV $454
ADDRGP4 $463
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 152
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+440
ADDRLP4 152
INDIRI4
ASGNI4
ADDRGP4 $466
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 156
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+444
ADDRLP4 156
INDIRI4
ASGNI4
ADDRGP4 $469
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 160
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+448
ADDRLP4 160
INDIRI4
ASGNI4
ADDRGP4 $472
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 164
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+464
ADDRLP4 164
INDIRI4
ASGNI4
ADDRGP4 $475
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 168
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+600
ADDRLP4 168
INDIRI4
ASGNI4
ADDRGP4 $478
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 172
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+488
ADDRLP4 172
INDIRI4
ASGNI4
ADDRGP4 $481
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 176
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+492
ADDRLP4 176
INDIRI4
ASGNI4
ADDRGP4 $484
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 180
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+496
ADDRLP4 180
INDIRI4
ASGNI4
ADDRGP4 $487
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 184
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+500
ADDRLP4 184
INDIRI4
ASGNI4
ADDRGP4 $490
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 188
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+504
ADDRLP4 188
INDIRI4
ASGNI4
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $491
ADDRGP4 $497
ARGP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 196
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 196
INDIRP4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 200
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+234996+508
ADDP4
ADDRLP4 200
INDIRI4
ASGNI4
LABELV $492
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 4
LTI4 $491
ADDRGP4 $500
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 204
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+408
ADDRLP4 204
INDIRI4
ASGNI4
ADDRGP4 $503
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 208
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+412
ADDRLP4 208
INDIRI4
ASGNI4
ADDRGP4 $506
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 212
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+572
ADDRLP4 212
INDIRI4
ASGNI4
ADDRGP4 $509
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 216
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+576
ADDRLP4 216
INDIRI4
ASGNI4
ADDRGP4 $512
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 220
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+580
ADDRLP4 220
INDIRI4
ASGNI4
ADDRGP4 $515
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 224
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+640
ADDRLP4 224
INDIRI4
ASGNI4
ADDRGP4 $518
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 228
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgs+234996+644
ADDRLP4 228
INDIRI4
ASGNI4
LABELV $372
endproc CG_RegisterSounds 232 16
data
align 4
LABELV $520
address $521
address $522
address $523
address $524
address $525
address $526
address $527
address $528
address $529
address $530
address $531
align 4
LABELV $532
address $533
address $534
address $535
address $536
address $537
address $538
address $539
address $540
code
proc CG_RegisterGraphics 256 16
ADDRGP4 cg+112072
ARGP4
CNSTI4 0
ARGI4
CNSTI4 368
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 trap_R_ClearScene
CALLV
pop
ADDRGP4 cgs+31476
ARGP4
ADDRGP4 trap_R_LoadWorldMap
CALLV
pop
CNSTF4 1059648963
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $544
ADDRLP4 4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ASGNI4
ADDRLP4 4
INDIRI4
ADDRGP4 $520
ADDP4
INDIRP4
ARGP4
ADDRLP4 8
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRLP4 4
INDIRI4
ADDRGP4 cgs+234996+164
ADDP4
ADDRLP4 8
INDIRI4
ASGNI4
LABELV $545
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 11
LTI4 $544
ADDRGP4 $552
ARGP4
ADDRLP4 12
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+24
ADDRLP4 12
INDIRI4
ASGNI4
ADDRGP4 $555
ARGP4
ADDRLP4 16
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+20
ADDRLP4 16
INDIRI4
ASGNI4
ADDRGP4 $558
ARGP4
ADDRLP4 20
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+144
ADDRLP4 20
INDIRI4
ASGNI4
ADDRGP4 $561
ARGP4
ADDRLP4 24
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+152
ADDRLP4 24
INDIRI4
ASGNI4
ADDRGP4 $564
ARGP4
ADDRLP4 28
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+156
ADDRLP4 28
INDIRI4
ASGNI4
ADDRGP4 $567
ARGP4
ADDRLP4 32
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+160
ADDRLP4 32
INDIRI4
ASGNI4
ADDRGP4 $570
ARGP4
ADDRLP4 36
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+28
ADDRLP4 36
INDIRI4
ASGNI4
ADDRGP4 $573
ARGP4
ADDRLP4 40
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+140
ADDRLP4 40
INDIRI4
ASGNI4
ADDRGP4 $576
ARGP4
ADDRLP4 44
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+216
ADDRLP4 44
INDIRI4
ASGNI4
ADDRGP4 $579
ARGP4
ADDRLP4 48
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+220
ADDRLP4 48
INDIRI4
ASGNI4
ADDRGP4 $582
ARGP4
ADDRLP4 52
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+224
ADDRLP4 52
INDIRI4
ASGNI4
ADDRGP4 $585
ARGP4
ADDRLP4 56
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+228
ADDRLP4 56
INDIRI4
ASGNI4
ADDRGP4 $588
ARGP4
ADDRLP4 60
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+232
ADDRLP4 60
INDIRI4
ASGNI4
ADDRGP4 $591
ARGP4
ADDRLP4 64
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+236
ADDRLP4 64
INDIRI4
ASGNI4
ADDRGP4 $594
ARGP4
ADDRLP4 68
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+240
ADDRLP4 68
INDIRI4
ASGNI4
ADDRGP4 $597
ARGP4
ADDRLP4 72
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+244
ADDRLP4 72
INDIRI4
ASGNI4
ADDRGP4 $600
ARGP4
ADDRLP4 76
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+248
ADDRLP4 76
INDIRI4
ASGNI4
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $601
ADDRGP4 $607
ARGP4
ADDRLP4 0
INDIRI4
CNSTI4 97
ADDI4
ARGI4
ADDRLP4 84
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 84
INDIRP4
ARGP4
ADDRLP4 88
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+234996+252
ADDP4
ADDRLP4 88
INDIRI4
ASGNI4
LABELV $602
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 4
LTI4 $601
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $608
ADDRLP4 92
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ASGNI4
ADDRLP4 92
INDIRI4
ADDRGP4 $532
ADDP4
INDIRP4
ARGP4
ADDRLP4 96
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRLP4 92
INDIRI4
ADDRGP4 cgs+234996+648
ADDP4
ADDRLP4 96
INDIRI4
ASGNI4
LABELV $609
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 8
LTI4 $608
ADDRGP4 $616
ARGP4
ADDRLP4 100
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+684
ADDRLP4 100
INDIRI4
ASGNI4
ADDRGP4 $619
ARGP4
ADDRLP4 104
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+688
ADDRLP4 104
INDIRI4
ASGNI4
ADDRGP4 $622
ARGP4
ADDRLP4 108
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+692
ADDRLP4 108
INDIRI4
ASGNI4
ADDRGP4 $625
ARGP4
ADDRLP4 112
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+696
ADDRLP4 112
INDIRI4
ASGNI4
ADDRGP4 $628
ARGP4
ADDRLP4 116
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+700
ADDRLP4 116
INDIRI4
ASGNI4
ADDRGP4 $631
ARGP4
ADDRLP4 120
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+680
ADDRLP4 120
INDIRI4
ASGNI4
ADDRGP4 $634
ARGP4
ADDRLP4 124
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+16
ADDRLP4 124
INDIRI4
ASGNI4
ADDRGP4 $637
ARGP4
ADDRLP4 128
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+268
ADDRLP4 128
INDIRI4
ASGNI4
CNSTF4 1060320051
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 cg_weapons
ARGP4
CNSTI4 0
ARGI4
CNSTI4 41600
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 cg_upgrades
ARGP4
CNSTI4 0
ARGI4
CNSTI4 384
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 $640
ARGP4
ADDRLP4 132
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+208
ADDRLP4 132
INDIRI4
ASGNI4
ADDRGP4 $643
ARGP4
ADDRLP4 136
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+212
ADDRLP4 136
INDIRI4
ASGNI4
ADDRGP4 $646
ARGP4
ADDRLP4 140
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+584
ADDRLP4 140
INDIRI4
ASGNI4
ADDRGP4 $649
ARGP4
ADDRLP4 144
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+588
ADDRLP4 144
INDIRI4
ASGNI4
ADDRGP4 $652
ARGP4
ADDRLP4 148
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+592
ADDRLP4 148
INDIRI4
ASGNI4
ADDRGP4 $655
ARGP4
ADDRLP4 152
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+596
ADDRLP4 152
INDIRI4
ASGNI4
ADDRGP4 $658
ARGP4
ADDRLP4 156
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+452
ADDRLP4 156
INDIRI4
ASGNI4
ADDRGP4 $661
ARGP4
ADDRLP4 160
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+456
ADDRLP4 160
INDIRI4
ASGNI4
ADDRGP4 $664
ARGP4
ADDRLP4 164
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+460
ADDRLP4 164
INDIRI4
ASGNI4
ADDRGP4 $667
ARGP4
ADDRLP4 168
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+604
ADDRLP4 168
INDIRI4
ASGNI4
ADDRGP4 $670
ARGP4
ADDRLP4 172
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+612
ADDRLP4 172
INDIRI4
ASGNI4
ADDRGP4 $673
ARGP4
ADDRLP4 176
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+608
ADDRLP4 176
INDIRI4
ASGNI4
ADDRGP4 $676
ARGP4
ADDRLP4 180
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+616
ADDRLP4 180
INDIRI4
ASGNI4
ADDRGP4 $679
ARGP4
ADDRLP4 184
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+632
ADDRLP4 184
INDIRI4
ASGNI4
ADDRGP4 $682
ARGP4
ADDRLP4 188
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+628
ADDRLP4 188
INDIRI4
ASGNI4
ADDRGP4 $685
ARGP4
ADDRLP4 192
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+620
ADDRLP4 192
INDIRI4
ASGNI4
ADDRGP4 $688
ARGP4
ADDRLP4 196
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRGP4 cgs+234996+624
ADDRLP4 196
INDIRI4
ASGNI4
ADDRGP4 $689
ARGP4
ADDRGP4 cgs+234676
ARGP4
ADDRGP4 CG_BuildableStatusParse
CALLV
pop
ADDRGP4 $691
ARGP4
ADDRGP4 cgs+234516
ARGP4
ADDRGP4 CG_BuildableStatusParse
CALLV
pop
ADDRGP4 $693
ARGP4
ADDRGP4 cgs+234836
ARGP4
ADDRGP4 CG_BuildableStatusParse
CALLV
pop
ADDRLP4 200
ADDRGP4 trap_CM_NumInlineModels
CALLI4
ASGNI4
ADDRGP4 cgs+37360
ADDRLP4 200
INDIRI4
ASGNI4
ADDRLP4 0
CNSTI4 1
ASGNI4
ADDRGP4 $699
JUMPV
LABELV $696
ADDRLP4 232
ARGP4
CNSTI4 10
ARGI4
ADDRGP4 $701
ARGP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRLP4 232
ARGP4
ADDRLP4 244
ADDRGP4 trap_R_RegisterModel
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+37364
ADDP4
ADDRLP4 244
INDIRI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+37364
ADDP4
INDIRI4
ARGI4
ADDRLP4 208
ARGP4
ADDRLP4 220
ARGP4
ADDRGP4 trap_R_ModelBounds
CALLV
pop
ADDRLP4 204
CNSTI4 0
ASGNI4
LABELV $704
ADDRLP4 248
ADDRLP4 204
INDIRI4
CNSTI4 2
LSHI4
ASGNI4
ADDRLP4 252
ADDRLP4 248
INDIRI4
ADDRLP4 208
ADDP4
INDIRF4
ASGNF4
ADDRLP4 248
INDIRI4
CNSTI4 12
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cgs+38388
ADDP4
ADDP4
ADDRLP4 252
INDIRF4
CNSTF4 1056964608
ADDRLP4 248
INDIRI4
ADDRLP4 220
ADDP4
INDIRF4
ADDRLP4 252
INDIRF4
SUBF4
MULF4
ADDF4
ASGNF4
LABELV $705
ADDRLP4 204
ADDRLP4 204
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 204
INDIRI4
CNSTI4 3
LTI4 $704
LABELV $697
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $699
ADDRLP4 0
INDIRI4
ADDRGP4 cgs+37360
INDIRI4
LTI4 $696
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $709
ADDRLP4 0
INDIRI4
CNSTI4 30
ADDI4
ARGI4
ADDRLP4 208
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 204
ADDRLP4 208
INDIRP4
ASGNP4
ADDRLP4 204
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $713
ADDRGP4 $711
JUMPV
LABELV $713
ADDRLP4 204
INDIRP4
ARGP4
ADDRLP4 212
ADDRGP4 trap_R_RegisterModel
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+34800
ADDP4
ADDRLP4 212
INDIRI4
ASGNI4
LABELV $710
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 256
LTI4 $709
LABELV $711
CNSTF4 1061997773
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $716
ADDRLP4 0
INDIRI4
CNSTI4 542
ADDI4
ARGI4
ADDRLP4 208
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 204
ADDRLP4 208
INDIRP4
ASGNP4
ADDRLP4 204
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $720
ADDRGP4 $718
JUMPV
LABELV $720
ADDRLP4 204
INDIRP4
ARGP4
ADDRLP4 212
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+35824
ADDP4
ADDRLP4 212
INDIRI4
ASGNI4
LABELV $717
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 64
LTI4 $716
LABELV $718
CNSTF4 1063675494
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $723
ADDRLP4 0
INDIRI4
CNSTI4 606
ADDI4
ARGI4
ADDRLP4 208
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 204
ADDRLP4 208
INDIRP4
ASGNP4
ADDRLP4 204
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $727
ADDRGP4 $725
JUMPV
LABELV $727
ADDRLP4 204
INDIRP4
ARGP4
ADDRLP4 212
ADDRGP4 CG_RegisterParticleSystem
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs+36080
ADDP4
ADDRLP4 212
INDIRI4
ASGNI4
LABELV $724
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 64
LTI4 $723
LABELV $725
LABELV $519
endproc CG_RegisterGraphics 256 16
export CG_BuildSpectatorString
proc CG_BuildSpectatorString 16 12
ADDRGP4 cg+115344
CNSTI1 0
ASGNI1
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $732
ADDRLP4 4
CNSTI4 1508
ADDRLP4 0
INDIRI4
MULI4
ASGNI4
ADDRLP4 8
CNSTI4 0
ASGNI4
ADDRLP4 4
INDIRI4
ADDRGP4 cgs+41460
ADDP4
INDIRI4
ADDRLP4 8
INDIRI4
EQI4 $736
ADDRLP4 4
INDIRI4
ADDRGP4 cgs+41460+68
ADDP4
INDIRI4
ADDRLP4 8
INDIRI4
NEI4 $736
ADDRGP4 $743
ARGP4
CNSTI4 1508
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cgs+41460+4
ADDP4
ARGP4
ADDRLP4 12
ADDRGP4 va
CALLP4
ASGNP4
ADDRGP4 cg+115344
ARGP4
CNSTI4 1024
ARGI4
ADDRLP4 12
INDIRP4
ARGP4
ADDRGP4 Q_strcat
CALLV
pop
LABELV $736
LABELV $733
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 64
LTI4 $732
LABELV $730
endproc CG_BuildSpectatorString 16 12
proc CG_RegisterClients 36 12
ADDRGP4 cg+119056
CNSTF4 0
ASGNF4
ADDRLP4 0
CNSTI4 1
ASGNI4
LABELV $748
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 4
ADDRGP4 BG_ClassConfig
CALLP4
ASGNP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 8
ADDRGP4 BG_ClassConfig
CALLP4
ASGNP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 4
INDIRP4
ARGP4
ADDRLP4 8
INDIRP4
CNSTI4 68
ADDP4
ARGP4
ADDRGP4 CG_PrecacheClientInfo
CALLV
pop
ADDRGP4 cg+119056
ADDRLP4 0
INDIRI4
CVIF4 4
CNSTF4 1095761920
DIVF4
ASGNF4
ADDRGP4 trap_UpdateScreen
CALLV
pop
LABELV $749
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 13
LTI4 $748
ADDRGP4 $755
ARGP4
ADDRLP4 4
ADDRGP4 trap_R_RegisterSkin
CALLI4
ASGNI4
ADDRGP4 cgs+234996+548
ADDRLP4 4
INDIRI4
ASGNI4
ADDRGP4 $758
ARGP4
ADDRLP4 8
ADDRGP4 trap_R_RegisterSkin
CALLI4
ASGNI4
ADDRGP4 cgs+234996+552
ADDRLP4 8
INDIRI4
ASGNI4
ADDRGP4 $761
ARGP4
ADDRLP4 12
ADDRGP4 trap_R_RegisterSkin
CALLI4
ASGNI4
ADDRGP4 cgs+234996+556
ADDRLP4 12
INDIRI4
ASGNI4
ADDRGP4 $764
ARGP4
ADDRLP4 16
ADDRGP4 trap_R_RegisterModel
CALLI4
ASGNI4
ADDRGP4 cgs+234996+560
ADDRLP4 16
INDIRI4
ASGNI4
ADDRGP4 $767
ARGP4
ADDRLP4 20
ADDRGP4 trap_R_RegisterModel
CALLI4
ASGNI4
ADDRGP4 cgs+234996+564
ADDRLP4 20
INDIRI4
ASGNI4
ADDRGP4 $770
ARGP4
ADDRLP4 24
ADDRGP4 trap_R_RegisterModel
CALLI4
ASGNI4
ADDRGP4 cgs+234996+568
ADDRLP4 24
INDIRI4
ASGNI4
ADDRGP4 cg+119056
CNSTF4 1065353216
ASGNF4
ADDRGP4 trap_UpdateScreen
CALLV
pop
ADDRLP4 0
CNSTI4 0
ASGNI4
LABELV $772
ADDRLP4 0
INDIRI4
CNSTI4 670
ADDI4
ARGI4
ADDRLP4 32
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 28
ADDRLP4 32
INDIRP4
ASGNP4
ADDRLP4 28
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $776
ADDRGP4 $773
JUMPV
LABELV $776
ADDRLP4 0
INDIRI4
ARGI4
ADDRGP4 CG_NewClientInfo
CALLV
pop
LABELV $773
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
INDIRI4
CNSTI4 64
LTI4 $772
ADDRGP4 CG_BuildSpectatorString
CALLV
pop
LABELV $746
endproc CG_RegisterClients 36 12
export CG_ConfigString
proc CG_ConfigString 8 8
ADDRFP4 0
ADDRFP4 0
INDIRI4
ASGNI4
ADDRFP4 0
INDIRI4
CNSTI4 0
LTI4 $781
ADDRFP4 0
INDIRI4
CNSTI4 1024
LTI4 $779
LABELV $781
ADDRGP4 $782
ARGP4
ADDRLP4 4
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 4
INDIRP4
ARGP4
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 CG_Error
CALLV
pop
LABELV $779
ADDRFP4 0
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 cgs
ADDP4
INDIRI4
ADDRGP4 cgs+4096
ADDP4
RETP4
LABELV $778
endproc CG_ConfigString 8 8
export CG_StartMusic
proc CG_StartMusic 144 12
CNSTI4 2
ARGI4
ADDRLP4 132
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 132
INDIRP4
ASGNP4
ADDRLP4 0
ARGP4
ADDRLP4 136
ADDRGP4 COM_Parse
CALLP4
ASGNP4
ADDRLP4 4
ARGP4
ADDRLP4 136
INDIRP4
ARGP4
CNSTI4 64
ARGI4
ADDRGP4 Q_strncpyz
CALLV
pop
ADDRLP4 0
ARGP4
ADDRLP4 140
ADDRGP4 COM_Parse
CALLP4
ASGNP4
ADDRLP4 68
ARGP4
ADDRLP4 140
INDIRP4
ARGP4
CNSTI4 64
ARGI4
ADDRGP4 Q_strncpyz
CALLV
pop
ADDRLP4 4
ARGP4
ADDRLP4 68
ARGP4
ADDRGP4 trap_S_StartBackgroundTrack
CALLV
pop
LABELV $784
endproc CG_StartMusic 144 12
export CG_PlayerCount
proc CG_PlayerCount 12 0
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRGP4 CG_RequestScores
CALLI4
pop
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $789
JUMPV
LABELV $786
ADDRLP4 8
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ASGNI4
ADDRLP4 8
INDIRI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
CNSTI4 1
EQI4 $797
ADDRLP4 8
INDIRI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
CNSTI4 2
NEI4 $791
LABELV $797
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $791
LABELV $787
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $789
ADDRLP4 0
INDIRI4
ADDRGP4 cg+113492
INDIRI4
LTI4 $786
ADDRLP4 4
INDIRI4
RETI4
LABELV $785
endproc CG_PlayerCount 12 0
bss
align 1
LABELV $799
skip 32768
export CG_GetMenuBuffer
code
proc CG_GetMenuBuffer 20 16
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 8
ADDRGP4 trap_FS_FOpenFile
CALLI4
ASGNI4
ADDRLP4 0
ADDRLP4 8
INDIRI4
ASGNI4
ADDRLP4 4
INDIRI4
CNSTI4 0
NEI4 $800
ADDRGP4 $802
ARGP4
ADDRLP4 12
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 12
INDIRP4
ARGP4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 16
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 16
INDIRP4
ARGP4
ADDRGP4 trap_Print
CALLV
pop
CNSTP4 0
RETP4
ADDRGP4 $798
JUMPV
LABELV $800
ADDRLP4 0
INDIRI4
CNSTI4 32768
LTI4 $803
ADDRGP4 $805
ARGP4
ADDRLP4 12
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 12
INDIRP4
ARGP4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0
INDIRI4
ARGI4
CNSTI4 32768
ARGI4
ADDRLP4 16
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 16
INDIRP4
ARGP4
ADDRGP4 trap_Print
CALLV
pop
ADDRLP4 4
INDIRI4
ARGI4
ADDRGP4 trap_FS_FCloseFile
CALLV
pop
CNSTP4 0
RETP4
ADDRGP4 $798
JUMPV
LABELV $803
ADDRGP4 $799
ARGP4
ADDRLP4 0
INDIRI4
ARGI4
ADDRLP4 4
INDIRI4
ARGI4
ADDRGP4 trap_FS_Read
CALLV
pop
ADDRLP4 0
INDIRI4
ADDRGP4 $799
ADDP4
CNSTI1 0
ASGNI1
ADDRLP4 4
INDIRI4
ARGI4
ADDRGP4 trap_FS_FCloseFile
CALLV
pop
ADDRGP4 $799
RETP4
LABELV $798
endproc CG_GetMenuBuffer 20 16
export CG_Asset_Parse
proc CG_Asset_Parse 1136 16
ADDRFP4 0
ADDRFP4 0
INDIRI4
ASGNI4
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 0
ARGP4
ADDRLP4 1044
ADDRGP4 trap_Parse_ReadToken
CALLI4
ASGNI4
ADDRLP4 1044
INDIRI4
CNSTI4 0
NEI4 $807
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $807
ADDRLP4 0+16
ARGP4
ADDRGP4 $812
ARGP4
ADDRLP4 1048
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1048
INDIRI4
CNSTI4 0
EQI4 $814
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $813
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 0
ARGP4
ADDRLP4 1052
ADDRGP4 trap_Parse_ReadToken
CALLI4
ASGNI4
ADDRLP4 1052
INDIRI4
CNSTI4 0
NEI4 $816
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $816
ADDRLP4 0+16
ARGP4
ADDRGP4 $821
ARGP4
ADDRLP4 1056
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1056
INDIRI4
CNSTI4 0
NEI4 $818
CNSTI4 1
RETI4
ADDRGP4 $806
JUMPV
LABELV $818
ADDRLP4 0+16
ARGP4
ADDRGP4 $825
ARGP4
ADDRLP4 1060
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1060
INDIRI4
CNSTI4 0
NEI4 $822
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1068
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1068
INDIRI4
CNSTI4 0
EQI4 $828
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1064
ARGP4
ADDRLP4 1072
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1072
INDIRI4
CNSTI4 0
NEI4 $826
LABELV $828
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $826
ADDRLP4 1040
INDIRP4
ARGP4
ADDRLP4 1064
INDIRI4
ARGI4
ADDRGP4 cgDC+244+12
ARGP4
ADDRGP4 cgDC+52
INDIRP4
CALLV
pop
ADDRGP4 $814
JUMPV
LABELV $822
ADDRLP4 0+16
ARGP4
ADDRGP4 $835
ARGP4
ADDRLP4 1064
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1064
INDIRI4
CNSTI4 0
NEI4 $832
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1072
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1072
INDIRI4
CNSTI4 0
EQI4 $838
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1068
ARGP4
ADDRLP4 1076
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1076
INDIRI4
CNSTI4 0
NEI4 $836
LABELV $838
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $836
ADDRLP4 1040
INDIRP4
ARGP4
ADDRLP4 1068
INDIRI4
ARGI4
ADDRGP4 cgDC+244+20560
ARGP4
ADDRGP4 cgDC+52
INDIRP4
CALLV
pop
ADDRGP4 $814
JUMPV
LABELV $832
ADDRLP4 0+16
ARGP4
ADDRGP4 $845
ARGP4
ADDRLP4 1068
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1068
INDIRI4
CNSTI4 0
NEI4 $842
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1076
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1076
INDIRI4
CNSTI4 0
EQI4 $848
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1072
ARGP4
ADDRLP4 1080
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1080
INDIRI4
CNSTI4 0
NEI4 $846
LABELV $848
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $846
ADDRLP4 1040
INDIRP4
ARGP4
ADDRLP4 1072
INDIRI4
ARGI4
ADDRGP4 cgDC+244+41108
ARGP4
ADDRGP4 cgDC+52
INDIRP4
CALLV
pop
ADDRGP4 $814
JUMPV
LABELV $842
ADDRLP4 0+16
ARGP4
ADDRGP4 $855
ARGP4
ADDRLP4 1072
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1072
INDIRI4
CNSTI4 0
NEI4 $852
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1080
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1080
INDIRI4
CNSTI4 0
EQI4 $858
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1076
ARGP4
ADDRLP4 1084
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1084
INDIRI4
CNSTI4 0
NEI4 $856
LABELV $858
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $856
ADDRLP4 1088
ADDRLP4 1040
INDIRP4
ASGNP4
ADDRLP4 1088
INDIRP4
ARGP4
ADDRLP4 1076
INDIRI4
ARGI4
ADDRLP4 1088
INDIRP4
ARGP4
ADDRGP4 cgDC+244+61656
ARGP4
ADDRGP4 cgDC+56
INDIRP4
CALLV
pop
ADDRGP4 $814
JUMPV
LABELV $852
ADDRLP4 0+16
ARGP4
ADDRGP4 $865
ARGP4
ADDRLP4 1076
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1076
INDIRI4
CNSTI4 0
NEI4 $862
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1084
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1084
INDIRI4
CNSTI4 0
EQI4 $868
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1080
ARGP4
ADDRLP4 1088
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1088
INDIRI4
CNSTI4 0
NEI4 $866
LABELV $868
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $866
ADDRLP4 1092
ADDRLP4 1040
INDIRP4
ASGNP4
ADDRLP4 1092
INDIRP4
ARGP4
ADDRLP4 1080
INDIRI4
ARGI4
ADDRLP4 1092
INDIRP4
ARGP4
ADDRGP4 cgDC+244+63268
ARGP4
ADDRGP4 cgDC+56
INDIRP4
CALLV
pop
ADDRGP4 $814
JUMPV
LABELV $862
ADDRLP4 0+16
ARGP4
ADDRGP4 $875
ARGP4
ADDRLP4 1080
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1080
INDIRI4
CNSTI4 0
NEI4 $872
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1088
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1088
INDIRI4
CNSTI4 0
EQI4 $878
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1084
ARGP4
ADDRLP4 1092
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1092
INDIRI4
CNSTI4 0
NEI4 $876
LABELV $878
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $876
ADDRLP4 1096
ADDRLP4 1040
INDIRP4
ASGNP4
ADDRLP4 1096
INDIRP4
ARGP4
ADDRLP4 1084
INDIRI4
ARGI4
ADDRLP4 1096
INDIRP4
ARGP4
ADDRGP4 cgDC+244+64880
ARGP4
ADDRGP4 cgDC+56
INDIRP4
CALLV
pop
ADDRGP4 $814
JUMPV
LABELV $872
ADDRLP4 0+16
ARGP4
ADDRGP4 $885
ARGP4
ADDRLP4 1084
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1084
INDIRI4
CNSTI4 0
NEI4 $882
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1088
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1088
INDIRI4
CNSTI4 0
NEI4 $886
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $886
ADDRLP4 1040
INDIRP4
ARGP4
ADDRLP4 1092
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66496
ADDRLP4 1092
INDIRI4
ASGNI4
ADDRGP4 $814
JUMPV
LABELV $882
ADDRLP4 0+16
ARGP4
ADDRGP4 $893
ARGP4
ADDRLP4 1088
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1088
INDIRI4
CNSTI4 0
NEI4 $890
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1092
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1092
INDIRI4
CNSTI4 0
NEI4 $894
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $894
ADDRLP4 1040
INDIRP4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 1096
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66544
ADDRLP4 1096
INDIRI4
ASGNI4
ADDRGP4 $814
JUMPV
LABELV $890
ADDRLP4 0+16
ARGP4
ADDRGP4 $901
ARGP4
ADDRLP4 1092
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1092
INDIRI4
CNSTI4 0
NEI4 $898
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1096
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1096
INDIRI4
CNSTI4 0
NEI4 $902
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $902
ADDRLP4 1040
INDIRP4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 1100
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66548
ADDRLP4 1100
INDIRI4
ASGNI4
ADDRGP4 $814
JUMPV
LABELV $898
ADDRLP4 0+16
ARGP4
ADDRGP4 $909
ARGP4
ADDRLP4 1096
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1096
INDIRI4
CNSTI4 0
NEI4 $906
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1100
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1100
INDIRI4
CNSTI4 0
NEI4 $910
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $910
ADDRLP4 1040
INDIRP4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 1104
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66556
ADDRLP4 1104
INDIRI4
ASGNI4
ADDRGP4 $814
JUMPV
LABELV $906
ADDRLP4 0+16
ARGP4
ADDRGP4 $917
ARGP4
ADDRLP4 1100
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1100
INDIRI4
CNSTI4 0
NEI4 $914
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 1040
ARGP4
ADDRLP4 1104
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1104
INDIRI4
CNSTI4 0
NEI4 $918
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $918
ADDRLP4 1040
INDIRP4
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 1108
ADDRGP4 trap_S_RegisterSound
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66552
ADDRLP4 1108
INDIRI4
ASGNI4
ADDRGP4 $814
JUMPV
LABELV $914
ADDRLP4 0+16
ARGP4
ADDRGP4 $925
ARGP4
ADDRLP4 1104
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1104
INDIRI4
CNSTI4 0
NEI4 $922
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+4
ARGP4
ADDRLP4 1108
ADDRGP4 PC_String_Parse
CALLI4
ASGNI4
ADDRLP4 1108
INDIRI4
CNSTI4 0
NEI4 $926
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $926
ADDRGP4 cgDC+244+4
INDIRP4
ARGP4
ADDRLP4 1112
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66492
ADDRLP4 1112
INDIRI4
ASGNI4
ADDRGP4 $814
JUMPV
LABELV $922
ADDRLP4 0+16
ARGP4
ADDRGP4 $937
ARGP4
ADDRLP4 1108
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1108
INDIRI4
CNSTI4 0
NEI4 $934
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+66560
ARGP4
ADDRLP4 1112
ADDRGP4 PC_Float_Parse
CALLI4
ASGNI4
ADDRLP4 1112
INDIRI4
CNSTI4 0
NEI4 $814
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $934
ADDRLP4 0+16
ARGP4
ADDRGP4 $945
ARGP4
ADDRLP4 1112
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1112
INDIRI4
CNSTI4 0
NEI4 $942
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+66564
ARGP4
ADDRLP4 1116
ADDRGP4 PC_Int_Parse
CALLI4
ASGNI4
ADDRLP4 1116
INDIRI4
CNSTI4 0
NEI4 $814
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $942
ADDRLP4 0+16
ARGP4
ADDRGP4 $953
ARGP4
ADDRLP4 1116
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1116
INDIRI4
CNSTI4 0
NEI4 $950
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+66568
ARGP4
ADDRLP4 1120
ADDRGP4 PC_Float_Parse
CALLI4
ASGNI4
ADDRLP4 1120
INDIRI4
CNSTI4 0
NEI4 $814
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $950
ADDRLP4 0+16
ARGP4
ADDRGP4 $961
ARGP4
ADDRLP4 1120
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1120
INDIRI4
CNSTI4 0
NEI4 $958
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+66572
ARGP4
ADDRLP4 1124
ADDRGP4 PC_Float_Parse
CALLI4
ASGNI4
ADDRLP4 1124
INDIRI4
CNSTI4 0
NEI4 $814
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $958
ADDRLP4 0+16
ARGP4
ADDRGP4 $969
ARGP4
ADDRLP4 1124
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1124
INDIRI4
CNSTI4 0
NEI4 $966
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+66576
ARGP4
ADDRLP4 1128
ADDRGP4 PC_Float_Parse
CALLI4
ASGNI4
ADDRLP4 1128
INDIRI4
CNSTI4 0
NEI4 $814
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $966
ADDRLP4 0+16
ARGP4
ADDRGP4 $977
ARGP4
ADDRLP4 1128
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1128
INDIRI4
CNSTI4 0
NEI4 $974
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 cgDC+244+66580
ARGP4
ADDRLP4 1132
ADDRGP4 PC_Color_Parse
CALLI4
ASGNI4
ADDRLP4 1132
INDIRI4
CNSTI4 0
NEI4 $978
CNSTI4 0
RETI4
ADDRGP4 $806
JUMPV
LABELV $978
ADDRGP4 cgDC+244+66596
ADDRGP4 cgDC+244+66580+12
INDIRF4
ASGNF4
LABELV $974
LABELV $814
ADDRGP4 $813
JUMPV
CNSTI4 0
RETI4
LABELV $806
endproc CG_Asset_Parse 1136 16
export CG_ParseMenu
proc CG_ParseMenu 1064 8
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 1044
ADDRGP4 trap_Parse_LoadSource
CALLI4
ASGNI4
ADDRLP4 1040
ADDRLP4 1044
INDIRI4
ASGNI4
ADDRLP4 1040
INDIRI4
CNSTI4 0
NEI4 $988
ADDRGP4 $990
ARGP4
ADDRLP4 1048
ADDRGP4 trap_Parse_LoadSource
CALLI4
ASGNI4
ADDRLP4 1040
ADDRLP4 1048
INDIRI4
ASGNI4
LABELV $988
ADDRLP4 1040
INDIRI4
CNSTI4 0
NEI4 $994
ADDRGP4 $987
JUMPV
LABELV $993
ADDRLP4 1040
INDIRI4
ARGI4
ADDRLP4 0
ARGP4
ADDRLP4 1052
ADDRGP4 trap_Parse_ReadToken
CALLI4
ASGNI4
ADDRLP4 1052
INDIRI4
CNSTI4 0
NEI4 $996
ADDRGP4 $995
JUMPV
LABELV $996
ADDRLP4 0+16
INDIRI1
CVII4 1
CNSTI4 125
NEI4 $998
ADDRGP4 $995
JUMPV
LABELV $998
ADDRLP4 0+16
ARGP4
ADDRGP4 $1004
ARGP4
ADDRLP4 1056
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1056
INDIRI4
CNSTI4 0
NEI4 $1001
ADDRLP4 1040
INDIRI4
ARGI4
ADDRLP4 1060
ADDRGP4 CG_Asset_Parse
CALLI4
ASGNI4
ADDRLP4 1060
INDIRI4
CNSTI4 0
EQI4 $995
ADDRGP4 $994
JUMPV
LABELV $1001
ADDRLP4 0+16
ARGP4
ADDRGP4 $1010
ARGP4
ADDRLP4 1060
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 1060
INDIRI4
CNSTI4 0
NEI4 $1007
ADDRLP4 1040
INDIRI4
ARGI4
ADDRGP4 Menu_New
CALLV
pop
LABELV $1007
LABELV $994
ADDRGP4 $993
JUMPV
LABELV $995
ADDRLP4 1040
INDIRI4
ARGI4
ADDRGP4 trap_Parse_FreeSource
CALLI4
pop
LABELV $987
endproc CG_ParseMenu 1064 8
export CG_Load_Menu
proc CG_Load_Menu 20 8
ADDRFP4 0
ADDRFP4 0
INDIRP4
ASGNP4
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 4
ADDRGP4 COM_ParseExt
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 4
INDIRP4
ASGNP4
ADDRLP4 0
INDIRP4
INDIRI1
CVII4 1
CNSTI4 123
EQI4 $1015
CNSTI4 0
RETI4
ADDRGP4 $1011
JUMPV
LABELV $1014
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 8
ADDRGP4 COM_ParseExt
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 8
INDIRP4
ASGNP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 $821
ARGP4
ADDRLP4 12
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 12
INDIRI4
CNSTI4 0
NEI4 $1017
CNSTI4 1
RETI4
ADDRGP4 $1011
JUMPV
LABELV $1017
ADDRLP4 0
INDIRP4
CVPU4 4
CNSTU4 0
EQU4 $1021
ADDRLP4 0
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $1019
LABELV $1021
CNSTI4 0
RETI4
ADDRGP4 $1011
JUMPV
LABELV $1019
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 CG_ParseMenu
CALLV
pop
LABELV $1015
ADDRGP4 $1014
JUMPV
CNSTI4 0
RETI4
LABELV $1011
endproc CG_Load_Menu 20 8
bss
align 1
LABELV $1023
skip 4096
export CG_LoadMenus
code
proc CG_LoadMenus 52 16
ADDRLP4 20
ADDRGP4 trap_Milliseconds
CALLI4
ASGNI4
ADDRLP4 16
ADDRLP4 20
INDIRI4
ASGNI4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 8
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 24
ADDRGP4 trap_FS_FOpenFile
CALLI4
ASGNI4
ADDRLP4 12
ADDRLP4 24
INDIRI4
ASGNI4
ADDRLP4 8
INDIRI4
CNSTI4 0
NEI4 $1024
ADDRGP4 $1026
ARGP4
ADDRLP4 28
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 28
INDIRP4
ARGP4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 32
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 32
INDIRP4
ARGP4
ADDRGP4 trap_Error
CALLV
pop
ADDRGP4 $224
ARGP4
ADDRLP4 8
ARGP4
CNSTI4 0
ARGI4
ADDRLP4 36
ADDRGP4 trap_FS_FOpenFile
CALLI4
ASGNI4
ADDRLP4 12
ADDRLP4 36
INDIRI4
ASGNI4
ADDRLP4 8
INDIRI4
CNSTI4 0
NEI4 $1027
ADDRGP4 $1029
ARGP4
ADDRLP4 40
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 40
INDIRP4
ARGP4
ADDRLP4 44
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 44
INDIRP4
ARGP4
ADDRGP4 trap_Error
CALLV
pop
LABELV $1027
LABELV $1024
ADDRLP4 12
INDIRI4
CNSTI4 4096
LTI4 $1030
ADDRGP4 $805
ARGP4
ADDRLP4 28
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 28
INDIRP4
ARGP4
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 12
INDIRI4
ARGI4
CNSTI4 4096
ARGI4
ADDRLP4 32
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 32
INDIRP4
ARGP4
ADDRGP4 trap_Error
CALLV
pop
ADDRLP4 8
INDIRI4
ARGI4
ADDRGP4 trap_FS_FCloseFile
CALLV
pop
ADDRGP4 $1022
JUMPV
LABELV $1030
ADDRGP4 $1023
ARGP4
ADDRLP4 12
INDIRI4
ARGI4
ADDRLP4 8
INDIRI4
ARGI4
ADDRGP4 trap_FS_Read
CALLV
pop
ADDRLP4 12
INDIRI4
ADDRGP4 $1023
ADDP4
CNSTI1 0
ASGNI1
ADDRLP4 8
INDIRI4
ARGI4
ADDRGP4 trap_FS_FCloseFile
CALLV
pop
ADDRGP4 $1023
ARGP4
ADDRGP4 COM_Compress
CALLI4
pop
ADDRGP4 Menu_Reset
CALLV
pop
ADDRLP4 4
ADDRGP4 $1023
ASGNP4
ADDRGP4 $1033
JUMPV
LABELV $1032
ADDRLP4 4
ARGP4
CNSTI4 1
ARGI4
ADDRLP4 28
ADDRGP4 COM_ParseExt
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 28
INDIRP4
ASGNP4
ADDRLP4 0
INDIRP4
CVPU4 4
CNSTU4 0
EQU4 $1038
ADDRLP4 36
ADDRLP4 0
INDIRP4
INDIRI1
CVII4 1
ASGNI4
ADDRLP4 36
INDIRI4
CNSTI4 0
EQI4 $1038
ADDRLP4 36
INDIRI4
CNSTI4 125
NEI4 $1035
LABELV $1038
ADDRGP4 $1034
JUMPV
LABELV $1035
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 $821
ARGP4
ADDRLP4 40
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 40
INDIRI4
CNSTI4 0
NEI4 $1039
ADDRGP4 $1034
JUMPV
LABELV $1039
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 $1043
ARGP4
ADDRLP4 44
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 44
INDIRI4
CNSTI4 0
NEI4 $1041
ADDRLP4 4
ARGP4
ADDRLP4 48
ADDRGP4 CG_Load_Menu
CALLI4
ASGNI4
ADDRLP4 48
INDIRI4
CNSTI4 0
EQI4 $1034
LABELV $1041
LABELV $1033
ADDRGP4 $1032
JUMPV
LABELV $1034
ADDRGP4 $1046
ARGP4
ADDRLP4 28
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 32
ADDRGP4 trap_Milliseconds
CALLI4
ASGNI4
ADDRLP4 28
INDIRP4
ARGP4
ADDRLP4 32
INDIRI4
ADDRLP4 16
INDIRI4
SUBI4
ARGI4
ADDRGP4 Com_Printf
CALLV
pop
LABELV $1022
endproc CG_LoadMenus 52 16
proc CG_OwnerDrawHandleKey 0 0
CNSTI4 0
RETI4
LABELV $1047
endproc CG_OwnerDrawHandleKey 0 0
proc CG_FeederCount 8 0
ADDRLP4 4
CNSTI4 0
ASGNI4
ADDRFP4 0
INDIRI4
CNSTI4 2
NEI4 $1049
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1054
JUMPV
LABELV $1051
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
CNSTI4 1
NEI4 $1056
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1056
LABELV $1052
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1054
ADDRLP4 0
INDIRI4
ADDRGP4 cg+113492
INDIRI4
LTI4 $1051
ADDRGP4 $1050
JUMPV
LABELV $1049
ADDRFP4 0
INDIRI4
CNSTI4 3
NEI4 $1060
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1065
JUMPV
LABELV $1062
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
CNSTI4 2
NEI4 $1067
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1067
LABELV $1063
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1065
ADDRLP4 0
INDIRI4
ADDRGP4 cg+113492
INDIRI4
LTI4 $1062
LABELV $1060
LABELV $1050
ADDRLP4 4
INDIRI4
RETI4
LABELV $1048
endproc CG_FeederCount 8 0
export CG_SetScoreSelection
proc CG_SetScoreSelection 32 16
ADDRLP4 16
ADDRFP4 0
INDIRP4
ASGNP4
ADDRLP4 4
ADDRGP4 cg+36
INDIRP4
CNSTI4 44
ADDP4
ASGNP4
ADDRLP4 24
CNSTI4 0
ASGNI4
ADDRLP4 12
ADDRLP4 24
INDIRI4
ASGNI4
ADDRLP4 8
ADDRLP4 24
INDIRI4
ASGNI4
ADDRLP4 0
ADDRLP4 24
INDIRI4
ASGNI4
ADDRGP4 $1076
JUMPV
LABELV $1073
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
CNSTI4 1
NEI4 $1078
ADDRLP4 8
ADDRLP4 8
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRGP4 $1079
JUMPV
LABELV $1078
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
CNSTI4 2
NEI4 $1082
ADDRLP4 12
ADDRLP4 12
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1082
LABELV $1079
ADDRLP4 4
INDIRP4
CNSTI4 148
ADDP4
INDIRI4
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508
ADDP4
INDIRI4
NEI4 $1086
ADDRGP4 cg+113496
ADDRLP4 0
INDIRI4
ASGNI4
LABELV $1086
LABELV $1074
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1076
ADDRLP4 0
INDIRI4
ADDRGP4 cg+113492
INDIRI4
LTI4 $1073
ADDRLP4 16
INDIRP4
CVPU4 4
CNSTU4 0
NEU4 $1090
ADDRGP4 $1071
JUMPV
LABELV $1090
ADDRLP4 28
CNSTI4 2
ASGNI4
ADDRLP4 20
ADDRLP4 28
INDIRI4
ASGNI4
ADDRLP4 0
ADDRLP4 8
INDIRI4
ASGNI4
CNSTI4 28
ADDRGP4 cg+113496
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
ADDRLP4 28
INDIRI4
NEI4 $1092
ADDRLP4 20
CNSTI4 3
ASGNI4
ADDRLP4 0
ADDRLP4 12
INDIRI4
ASGNI4
LABELV $1092
ADDRLP4 16
INDIRP4
ARGP4
ADDRLP4 20
INDIRI4
ARGI4
ADDRLP4 0
INDIRI4
ARGI4
CNSTP4 0
ARGP4
ADDRGP4 Menu_SetFeederSelection
CALLV
pop
LABELV $1071
endproc CG_SetScoreSelection 32 16
proc CG_InfoFromScoreIndex 12 0
ADDRFP4 0
ADDRFP4 0
INDIRI4
ASGNI4
ADDRFP4 4
ADDRFP4 4
INDIRI4
ASGNI4
ADDRFP4 8
ADDRFP4 8
INDIRP4
ASGNP4
ADDRLP4 8
CNSTI4 0
ASGNI4
ADDRLP4 4
ADDRLP4 8
INDIRI4
ASGNI4
ADDRLP4 0
ADDRLP4 8
INDIRI4
ASGNI4
ADDRGP4 $1101
JUMPV
LABELV $1098
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
ADDRFP4 4
INDIRI4
NEI4 $1103
ADDRLP4 4
INDIRI4
ADDRFP4 0
INDIRI4
NEI4 $1107
ADDRFP4 8
INDIRP4
ADDRLP4 0
INDIRI4
ASGNI4
CNSTI4 1508
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508
ADDP4
INDIRI4
MULI4
ADDRGP4 cgs+41460
ADDP4
RETP4
ADDRGP4 $1097
JUMPV
LABELV $1107
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1103
LABELV $1099
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1101
ADDRLP4 0
INDIRI4
ADDRGP4 cg+113492
INDIRI4
LTI4 $1098
ADDRFP4 8
INDIRP4
ADDRFP4 0
INDIRI4
ASGNI4
CNSTI4 1508
CNSTI4 28
ADDRFP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508
ADDP4
INDIRI4
MULI4
ADDRGP4 cgs+41460
ADDP4
RETP4
LABELV $1097
endproc CG_InfoFromScoreIndex 12 0
proc CG_ClientIsReady 16 8
CNSTI4 27
ARGI4
ADDRLP4 8
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 0
ARGP4
ADDRLP4 8
INDIRP4
ARGP4
ADDRGP4 Com_ClientListParse
CALLV
pop
ADDRLP4 0
ARGP4
ADDRFP4 0
INDIRI4
ARGI4
ADDRLP4 12
ADDRGP4 Com_ClientListContains
CALLI4
ASGNI4
ADDRLP4 12
INDIRI4
RETI4
LABELV $1113
endproc CG_ClientIsReady 16 8
proc CG_FeederItemText 60 12
ADDRLP4 12
CNSTI4 0
ASGNI4
ADDRLP4 0
CNSTP4 0
ASGNP4
ADDRLP4 8
CNSTI4 -1
ASGNI4
ADDRLP4 4
CNSTP4 0
ASGNP4
ADDRLP4 16
CNSTI4 0
ASGNI4
ADDRFP4 12
INDIRP4
CNSTI4 -1
ASGNI4
ADDRFP4 0
INDIRI4
CNSTI4 2
NEI4 $1115
ADDRLP4 8
CNSTI4 1
ASGNI4
ADDRGP4 $1116
JUMPV
LABELV $1115
ADDRFP4 0
INDIRI4
CNSTI4 3
NEI4 $1117
ADDRLP4 8
CNSTI4 2
ASGNI4
LABELV $1117
LABELV $1116
ADDRFP4 4
INDIRI4
ARGI4
ADDRLP4 8
INDIRI4
ARGI4
ADDRLP4 12
ARGP4
ADDRLP4 20
ADDRGP4 CG_InfoFromScoreIndex
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 20
INDIRP4
ASGNP4
ADDRLP4 4
CNSTI4 28
ADDRLP4 12
INDIRI4
MULI4
ADDRGP4 cg+113508
ADDP4
ASGNP4
ADDRGP4 cg+24
INDIRI4
CNSTI4 0
EQI4 $1120
ADDRLP4 4
INDIRP4
INDIRI4
ARGI4
ADDRLP4 24
ADDRGP4 CG_ClientIsReady
CALLI4
ASGNI4
ADDRLP4 24
INDIRI4
CNSTI4 0
EQI4 $1120
ADDRLP4 16
CNSTI4 0
ASGNI4
ADDRGP4 $1121
JUMPV
LABELV $1120
ADDRGP4 cg+36
INDIRP4
CNSTI4 48
ADDP4
INDIRI4
CNSTI4 2
EQI4 $1131
ADDRLP4 28
CNSTI4 0
ASGNI4
ADDRGP4 cg+36
INDIRP4
CNSTI4 56
ADDP4
INDIRI4
CNSTI4 2048
BANDI4
ADDRLP4 28
INDIRI4
NEI4 $1131
ADDRLP4 8
INDIRI4
ADDRGP4 cg+36
INDIRP4
CNSTI4 260
ADDP4
INDIRI4
EQI4 $1131
ADDRGP4 cg+24
INDIRI4
ADDRLP4 28
INDIRI4
EQI4 $1123
LABELV $1131
ADDRLP4 16
CNSTI4 1
ASGNI4
LABELV $1123
LABELV $1121
ADDRLP4 0
INDIRP4
CVPU4 4
CNSTU4 0
EQU4 $1132
ADDRLP4 0
INDIRP4
INDIRI4
CNSTI4 0
EQI4 $1132
ADDRLP4 36
ADDRFP4 8
INDIRI4
ASGNI4
ADDRLP4 36
INDIRI4
CNSTI4 0
LTI4 $1134
ADDRLP4 36
INDIRI4
CNSTI4 6
GTI4 $1134
ADDRLP4 36
INDIRI4
CNSTI4 2
LSHI4
ADDRGP4 $1170
ADDP4
INDIRP4
JUMPV
lit
align 4
LABELV $1170
address $1136
address $1142
address $1157
address $1162
address $1163
address $1165
address $1167
code
LABELV $1136
ADDRLP4 16
INDIRI4
CNSTI4 0
EQI4 $1135
ADDRLP4 4
INDIRP4
CNSTI4 20
ADDP4
INDIRI4
CNSTI4 0
EQI4 $1135
ADDRFP4 12
INDIRP4
CNSTI4 1300
ADDRLP4 4
INDIRP4
CNSTI4 20
ADDP4
INDIRI4
MULI4
ADDRGP4 cg_weapons+416
ADDP4
INDIRI4
ASGNI4
ADDRGP4 $1135
JUMPV
LABELV $1142
ADDRLP4 16
INDIRI4
CNSTI4 0
EQI4 $1135
ADDRLP4 4
INDIRP4
CNSTI4 16
ADDP4
INDIRI4
CNSTI4 2
NEI4 $1145
ADDRLP4 4
INDIRP4
CNSTI4 24
ADDP4
INDIRI4
CNSTI4 0
EQI4 $1145
ADDRFP4 12
INDIRP4
CNSTI4 12
ADDRLP4 4
INDIRP4
CNSTI4 24
ADDP4
INDIRI4
MULI4
ADDRGP4 cg_upgrades+8
ADDP4
INDIRI4
ASGNI4
ADDRGP4 $1135
JUMPV
LABELV $1145
ADDRLP4 4
INDIRP4
CNSTI4 16
ADDP4
INDIRI4
CNSTI4 1
NEI4 $1135
ADDRLP4 44
ADDRLP4 4
INDIRP4
CNSTI4 20
ADDP4
INDIRI4
ASGNI4
ADDRLP4 52
CNSTI4 3
ASGNI4
ADDRLP4 44
INDIRI4
ADDRLP4 52
INDIRI4
EQI4 $1153
ADDRLP4 44
INDIRI4
CNSTI4 5
EQI4 $1153
ADDRLP4 44
INDIRI4
CNSTI4 7
EQI4 $1153
ADDRLP4 44
INDIRI4
ADDRLP4 52
INDIRI4
LTI4 $1135
LABELV $1156
ADDRLP4 44
INDIRI4
CNSTI4 25
EQI4 $1153
ADDRGP4 $1135
JUMPV
LABELV $1153
ADDRFP4 12
INDIRP4
ADDRGP4 cgs+234996+680
INDIRI4
ASGNI4
ADDRGP4 $1135
JUMPV
LABELV $1157
ADDRGP4 cg+24
INDIRI4
CNSTI4 0
EQI4 $1135
ADDRLP4 4
INDIRP4
INDIRI4
ARGI4
ADDRLP4 40
ADDRGP4 CG_ClientIsReady
CALLI4
ASGNI4
ADDRLP4 40
INDIRI4
CNSTI4 0
EQI4 $1135
ADDRGP4 $1161
ARGP4
ADDRLP4 44
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 44
INDIRP4
RETP4
ADDRGP4 $1114
JUMPV
LABELV $1162
ADDRLP4 0
INDIRP4
CNSTI4 4
ADDP4
RETP4
ADDRGP4 $1114
JUMPV
LABELV $1163
ADDRGP4 $1164
ARGP4
ADDRLP4 4
INDIRP4
CNSTI4 4
ADDP4
INDIRI4
ARGI4
ADDRLP4 48
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 48
INDIRP4
RETP4
ADDRGP4 $1114
JUMPV
LABELV $1165
ADDRGP4 $1166
ARGP4
ADDRLP4 4
INDIRP4
CNSTI4 12
ADDP4
INDIRI4
ARGI4
ADDRLP4 52
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 52
INDIRP4
RETP4
ADDRGP4 $1114
JUMPV
LABELV $1167
ADDRLP4 4
INDIRP4
CNSTI4 8
ADDP4
INDIRI4
CNSTI4 -1
NEI4 $1168
ADDRGP4 $226
RETP4
ADDRGP4 $1114
JUMPV
LABELV $1168
ADDRGP4 $1166
ARGP4
ADDRLP4 4
INDIRP4
CNSTI4 8
ADDP4
INDIRI4
ARGI4
ADDRLP4 56
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 56
INDIRP4
RETP4
ADDRGP4 $1114
JUMPV
LABELV $1134
LABELV $1135
LABELV $1132
ADDRGP4 $226
RETP4
LABELV $1114
endproc CG_FeederItemText 60 12
proc CG_FeederItemImage 0 0
CNSTI4 0
RETI4
LABELV $1171
endproc CG_FeederItemImage 0 0
proc CG_FeederSelection 20 0
ADDRFP4 4
ADDRFP4 4
INDIRI4
ASGNI4
ADDRFP4 0
INDIRI4
CNSTI4 2
NEI4 $1174
ADDRLP4 12
CNSTI4 1
ASGNI4
ADDRGP4 $1175
JUMPV
LABELV $1174
ADDRLP4 12
CNSTI4 2
ASGNI4
LABELV $1175
ADDRLP4 8
ADDRLP4 12
INDIRI4
ASGNI4
ADDRLP4 16
CNSTI4 0
ASGNI4
ADDRLP4 4
ADDRLP4 16
INDIRI4
ASGNI4
ADDRLP4 0
ADDRLP4 16
INDIRI4
ASGNI4
ADDRGP4 $1179
JUMPV
LABELV $1176
CNSTI4 28
ADDRLP4 0
INDIRI4
MULI4
ADDRGP4 cg+113508+16
ADDP4
INDIRI4
ADDRLP4 8
INDIRI4
NEI4 $1181
ADDRFP4 4
INDIRI4
ADDRLP4 4
INDIRI4
NEI4 $1185
ADDRGP4 cg+113496
ADDRLP4 0
INDIRI4
ASGNI4
LABELV $1185
ADDRLP4 4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1181
LABELV $1177
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1179
ADDRLP4 0
INDIRI4
ADDRGP4 cg+113492
INDIRI4
LTI4 $1176
LABELV $1172
endproc CG_FeederSelection 20 0
proc CG_Cvar_Get 132 12
ADDRLP4 0
ARGP4
CNSTI4 0
ARGI4
CNSTI4 128
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRFP4 0
INDIRP4
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 128
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
ARGP4
ADDRLP4 128
ADDRGP4 atof
CALLF4
ASGNF4
ADDRLP4 128
INDIRF4
RETF4
LABELV $1188
endproc CG_Cvar_Get 132 12
export CG_Text_PaintWithCursor
proc CG_Text_PaintWithCursor 0 32
ADDRFP4 24
ADDRFP4 24
INDIRI4
CVII1 4
ASGNI1
ADDRFP4 0
INDIRF4
ARGF4
ADDRFP4 4
INDIRF4
ARGF4
ADDRFP4 8
INDIRF4
ARGF4
ADDRFP4 12
INDIRP4
ARGP4
ADDRFP4 16
INDIRP4
ARGP4
CNSTF4 0
ARGF4
ADDRFP4 28
INDIRI4
ARGI4
ADDRFP4 32
INDIRI4
ARGI4
ADDRGP4 UI_Text_Paint
CALLV
pop
LABELV $1189
endproc CG_Text_PaintWithCursor 0 32
proc CG_OwnerDrawWidth 8 12
ADDRFP4 0
INDIRI4
CNSTI4 31
EQI4 $1193
ADDRGP4 $1191
JUMPV
LABELV $1193
ADDRLP4 0
ADDRGP4 CG_GetKillerText
CALLP4
ASGNP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRF4
ARGF4
CNSTI4 0
ARGI4
ADDRLP4 4
ADDRGP4 UI_Text_Width
CALLF4
ASGNF4
ADDRLP4 4
INDIRF4
CVFI4 4
RETI4
ADDRGP4 $1190
JUMPV
LABELV $1191
CNSTI4 0
RETI4
LABELV $1190
endproc CG_OwnerDrawWidth 8 12
proc CG_PlayCinematic 4 24
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRF4
CVFI4 4
ARGI4
ADDRFP4 8
INDIRF4
CVFI4 4
ARGI4
ADDRFP4 12
INDIRF4
CVFI4 4
ARGI4
ADDRFP4 16
INDIRF4
CVFI4 4
ARGI4
CNSTI4 2
ARGI4
ADDRLP4 0
ADDRGP4 trap_CIN_PlayCinematic
CALLI4
ASGNI4
ADDRLP4 0
INDIRI4
RETI4
LABELV $1194
endproc CG_PlayCinematic 4 24
proc CG_StopCinematic 0 4
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 trap_CIN_StopCinematic
CALLI4
pop
LABELV $1195
endproc CG_StopCinematic 0 4
proc CG_DrawCinematic 0 20
ADDRFP4 0
INDIRI4
ARGI4
ADDRFP4 4
INDIRF4
CVFI4 4
ARGI4
ADDRFP4 8
INDIRF4
CVFI4 4
ARGI4
ADDRFP4 12
INDIRF4
CVFI4 4
ARGI4
ADDRFP4 16
INDIRF4
CVFI4 4
ARGI4
ADDRGP4 trap_CIN_SetExtents
CALLV
pop
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 trap_CIN_DrawCinematic
CALLV
pop
LABELV $1196
endproc CG_DrawCinematic 0 20
proc CG_RunCinematicFrame 0 4
ADDRFP4 0
INDIRI4
ARGI4
ADDRGP4 trap_CIN_RunCinematic
CALLI4
pop
LABELV $1197
endproc CG_RunCinematicFrame 0 4
proc CG_OwnerDrawVisible 0 0
CNSTI4 0
RETI4
LABELV $1198
endproc CG_OwnerDrawVisible 0 0
export CG_LoadHudMenu
proc CG_LoadHudMenu 1028 12
ADDRGP4 cgDC+220
CNSTF4 1142947840
ADDRGP4 cgs+20100+11308
INDIRI4
CVIF4 4
MULF4
CNSTF4 1139802112
ADDRGP4 cgs+20100+11304
INDIRI4
CVIF4 4
MULF4
DIVF4
ASGNF4
ADDRGP4 cgDC+216
ADDRGP4 cgs+20100+11304
INDIRI4
CVIF4 4
CNSTF4 1142947840
DIVF4
ASGNF4
ADDRGP4 cgDC+212
ADDRGP4 cgs+20100+11308
INDIRI4
CVIF4 4
CNSTF4 1139802112
DIVF4
ASGNF4
ADDRGP4 cgDC
ADDRGP4 trap_R_RegisterShaderNoMip
ASGNP4
ADDRGP4 cgDC+4
ADDRGP4 trap_R_SetColor
ASGNP4
ADDRGP4 cgDC+8
ADDRGP4 CG_DrawPic
ASGNP4
ADDRGP4 cgDC+12
ADDRGP4 trap_R_DrawStretchPic
ASGNP4
ADDRGP4 cgDC+16
ADDRGP4 trap_R_RegisterModel
ASGNP4
ADDRGP4 cgDC+20
ADDRGP4 trap_R_ModelBounds
ASGNP4
ADDRGP4 cgDC+24
ADDRGP4 CG_FillRect
ASGNP4
ADDRGP4 cgDC+28
ADDRGP4 CG_DrawRect
ASGNP4
ADDRGP4 cgDC+32
ADDRGP4 CG_DrawSides
ASGNP4
ADDRGP4 cgDC+36
ADDRGP4 CG_DrawTopBottom
ASGNP4
ADDRGP4 cgDC+40
ADDRGP4 trap_R_ClearScene
ASGNP4
ADDRGP4 cgDC+44
ADDRGP4 trap_R_AddRefEntityToScene
ASGNP4
ADDRGP4 cgDC+48
ADDRGP4 trap_R_RenderScene
ASGNP4
ADDRGP4 cgDC+52
ADDRGP4 trap_R_RegisterFont
ASGNP4
ADDRGP4 cgDC+56
ADDRGP4 LoadFace
ASGNP4
ADDRGP4 cgDC+60
ADDRGP4 FreeFace
ASGNP4
ADDRGP4 cgDC+64
ADDRGP4 LoadGlyph
ASGNP4
ADDRGP4 cgDC+68
ADDRGP4 FreeGlyph
ASGNP4
ADDRGP4 cgDC+72
ADDRGP4 Glyph
ASGNP4
ADDRGP4 cgDC+76
ADDRGP4 FreeCachedGlyphs
ASGNP4
ADDRGP4 cgDC+80
ADDRGP4 CG_OwnerDraw
ASGNP4
ADDRGP4 cgDC+84
ADDRGP4 CG_GetValue
ASGNP4
ADDRGP4 cgDC+88
ADDRGP4 CG_OwnerDrawVisible
ASGNP4
ADDRGP4 cgDC+92
ADDRGP4 CG_RunMenuScript
ASGNP4
ADDRGP4 cgDC+104
ADDRGP4 trap_Cvar_Set
ASGNP4
ADDRGP4 cgDC+96
ADDRGP4 trap_Cvar_VariableStringBuffer
ASGNP4
ADDRGP4 cgDC+100
ADDRGP4 CG_Cvar_Get
ASGNP4
ADDRGP4 cgDC+112
ADDRGP4 trap_Key_SetOverstrikeMode
ASGNP4
ADDRGP4 cgDC+116
ADDRGP4 trap_Key_GetOverstrikeMode
ASGNP4
ADDRGP4 cgDC+120
ADDRGP4 trap_S_StartLocalSound
ASGNP4
ADDRGP4 cgDC+124
ADDRGP4 CG_OwnerDrawHandleKey
ASGNP4
ADDRGP4 cgDC+128
ADDRGP4 CG_FeederCount
ASGNP4
ADDRGP4 cgDC+136
ADDRGP4 CG_FeederItemImage
ASGNP4
ADDRGP4 cgDC+132
ADDRGP4 CG_FeederItemText
ASGNP4
ADDRGP4 cgDC+140
ADDRGP4 CG_FeederSelection
ASGNP4
ADDRGP4 cgDC+164
ADDRGP4 Com_Error
ASGNP4
ADDRGP4 cgDC+168
ADDRGP4 Com_Printf
ASGNP4
ADDRGP4 cgDC+176
ADDRGP4 CG_OwnerDrawWidth
ASGNP4
ADDRGP4 cgDC+184
ADDRGP4 trap_S_RegisterSound
ASGNP4
ADDRGP4 cgDC+188
ADDRGP4 trap_S_StartBackgroundTrack
ASGNP4
ADDRGP4 cgDC+192
ADDRGP4 trap_S_StopBackgroundTrack
ASGNP4
ADDRGP4 cgDC+196
ADDRGP4 CG_PlayCinematic
ASGNP4
ADDRGP4 cgDC+200
ADDRGP4 CG_StopCinematic
ASGNP4
ADDRGP4 cgDC+204
ADDRGP4 CG_DrawCinematic
ASGNP4
ADDRGP4 cgDC+208
ADDRGP4 CG_RunCinematicFrame
ASGNP4
ADDRGP4 cgDC
ARGP4
ADDRGP4 Init_Display
CALLV
pop
ADDRGP4 Menu_Reset
CALLV
pop
ADDRGP4 $223
ARGP4
ADDRLP4 4
ARGP4
CNSTI4 1024
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
ADDRLP4 4
ASGNP4
ADDRLP4 0
INDIRP4
INDIRI1
CVII4 1
CNSTI4 0
NEI4 $1255
ADDRLP4 0
ADDRGP4 $224
ASGNP4
LABELV $1255
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 CG_LoadMenus
CALLV
pop
LABELV $1199
endproc CG_LoadHudMenu 1028 12
export CG_AssetCache
proc CG_AssetCache 52 12
ADDRGP4 $1260
ARGP4
ADDRLP4 4
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66496
ADDRLP4 4
INDIRI4
ASGNI4
ADDRGP4 $1263
ARGP4
ADDRLP4 8
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66516
ADDRLP4 8
INDIRI4
ASGNI4
ADDRGP4 $1266
ARGP4
ADDRLP4 12
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66504
ADDRLP4 12
INDIRI4
ASGNI4
ADDRGP4 $1269
ARGP4
ADDRLP4 16
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66500
ADDRLP4 16
INDIRI4
ASGNI4
ADDRGP4 $1272
ARGP4
ADDRLP4 20
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66508
ADDRLP4 20
INDIRI4
ASGNI4
ADDRGP4 $1275
ARGP4
ADDRLP4 24
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66512
ADDRLP4 24
INDIRI4
ASGNI4
ADDRGP4 $1278
ARGP4
ADDRLP4 28
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66520
ADDRLP4 28
INDIRI4
ASGNI4
ADDRGP4 $1281
ARGP4
ADDRLP4 32
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66536
ADDRLP4 32
INDIRI4
ASGNI4
ADDRGP4 $1284
ARGP4
ADDRLP4 36
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRGP4 cgDC+244+66540
ADDRLP4 36
INDIRI4
ASGNI4
ADDRGP4 cg_emoticons+12
INDIRI4
CNSTI4 0
EQI4 $1285
ADDRGP4 cgDC+244+66608
ARGP4
CNSTI4 64
ARGI4
ADDRLP4 40
ADDRGP4 BG_LoadEmoticons
CALLI4
ASGNI4
ADDRGP4 cgDC+244+68144
ADDRLP4 40
INDIRI4
ASGNI4
ADDRGP4 $1286
JUMPV
LABELV $1285
ADDRGP4 cgDC+244+68144
CNSTI4 0
ASGNI4
LABELV $1286
ADDRLP4 0
CNSTI4 0
ASGNI4
ADDRGP4 $1297
JUMPV
LABELV $1294
ADDRGP4 $1303
ARGP4
ADDRLP4 40
CNSTI4 24
ADDRLP4 0
INDIRI4
MULI4
ASGNI4
ADDRLP4 40
INDIRI4
ADDRGP4 cgDC+244+66608
ADDP4
ARGP4
ADDRLP4 40
INDIRI4
ADDRGP4 cgDC+244+66608+16
ADDP4
INDIRI4
ARGI4
ADDRLP4 44
ADDRGP4 va
CALLP4
ASGNP4
ADDRLP4 44
INDIRP4
ARGP4
ADDRLP4 48
ADDRGP4 trap_R_RegisterShaderNoMip
CALLI4
ASGNI4
ADDRLP4 40
INDIRI4
ADDRGP4 cgDC+244+66608+20
ADDP4
ADDRLP4 48
INDIRI4
ASGNI4
LABELV $1295
ADDRLP4 0
ADDRLP4 0
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1297
ADDRLP4 0
INDIRI4
ADDRGP4 cgDC+244+68144
INDIRI4
LTI4 $1294
LABELV $1257
endproc CG_AssetCache 52 12
export CG_Init
proc CG_Init 52 12
ADDRGP4 cgs
ARGP4
CNSTI4 0
ARGI4
CNSTI4 235712
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 cg
ARGP4
CNSTI4 0
ARGI4
CNSTI4 155320
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 cg+110008
ARGP4
CNSTI4 0
ARGI4
CNSTI4 8
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 cg_entities
ARGP4
CNSTI4 0
ARGI4
CNSTI4 1888256
ARGI4
ADDRGP4 memset
CALLP4
pop
ADDRGP4 cg+4
ADDRFP4 8
INDIRI4
ASGNI4
ADDRGP4 cgs+31460
ADDRFP4 0
INDIRI4
ASGNI4
ADDRGP4 cgs+31456
ADDRFP4 4
INDIRI4
ASGNI4
ADDRGP4 cgs+20100
ARGP4
ADDRGP4 trap_GetGlconfig
CALLV
pop
ADDRGP4 cgs+31444
ADDRGP4 cgs+20100+11304
INDIRI4
CVIF4 4
CNSTF4 1142947840
DIVF4
ASGNF4
ADDRGP4 cgs+31448
ADDRGP4 cgs+20100+11308
INDIRI4
CVIF4 4
CNSTF4 1139802112
DIVF4
ASGNF4
ADDRGP4 $1324
ARGP4
ADDRLP4 4
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+4
ADDRLP4 4
INDIRI4
ASGNI4
ADDRGP4 $1326
ARGP4
ADDRLP4 8
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996
ADDRLP4 8
INDIRI4
ASGNI4
ADDRGP4 $1329
ARGP4
ADDRLP4 12
ADDRGP4 trap_R_RegisterShader
CALLI4
ASGNI4
ADDRGP4 cgs+234996+8
ADDRLP4 12
INDIRI4
ASGNI4
ADDRGP4 BG_InitClassConfigs
CALLV
pop
ADDRGP4 BG_InitBuildableConfigs
CALLV
pop
ADDRGP4 BG_InitAllowedGameElements
CALLV
pop
ADDRGP4 BG_InitMemory
CALLV
pop
ADDRGP4 CG_RegisterCvars
CALLV
pop
ADDRGP4 CG_InitConsoleCommands
CALLV
pop
ADDRGP4 String_Init
CALLV
pop
ADDRGP4 CG_AssetCache
CALLV
pop
ADDRGP4 CG_LoadHudMenu
CALLV
pop
ADDRGP4 cg+111972
CNSTI4 0
ASGNI4
ADDRGP4 cgs
ARGP4
ADDRGP4 trap_GetGameState
CALLV
pop
CNSTI4 9
ARGI4
ADDRLP4 16
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRGP4 cgs+31688
ARGP4
ADDRLP4 16
INDIRP4
ARGP4
CNSTI4 3072
ARGI4
ADDRGP4 Q_strncpyz
CALLV
pop
CNSTI4 10
ARGI4
ADDRLP4 20
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRGP4 cgs+31688+1024
ARGP4
ADDRLP4 20
INDIRP4
ARGP4
CNSTI4 1024
ARGI4
ADDRGP4 Q_strncpyz
CALLV
pop
CNSTI4 10
ARGI4
ADDRLP4 24
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRGP4 cgs+31688+2048
ARGP4
ADDRLP4 24
INDIRP4
ARGP4
CNSTI4 1024
ARGI4
ADDRGP4 Q_strncpyz
CALLV
pop
CNSTI4 21
ARGI4
ADDRLP4 28
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 28
INDIRP4
ASGNP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 $1343
ARGP4
ADDRLP4 32
ADDRGP4 strcmp
CALLI4
ASGNI4
ADDRLP4 32
INDIRI4
CNSTI4 0
EQI4 $1341
ADDRGP4 $1344
ARGP4
ADDRLP4 36
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 36
INDIRP4
ARGP4
ADDRGP4 $1343
ARGP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRGP4 CG_Error
CALLV
pop
LABELV $1341
CNSTI4 22
ARGI4
ADDRLP4 40
ADDRGP4 CG_ConfigString
CALLP4
ASGNP4
ADDRLP4 0
ADDRLP4 40
INDIRP4
ASGNP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRLP4 44
ADDRGP4 atoi
CALLI4
ASGNI4
ADDRGP4 cgs+34760
ADDRLP4 44
INDIRI4
ASGNI4
ADDRGP4 CG_ParseServerinfo
CALLV
pop
ADDRGP4 cgs+31476
ARGP4
ADDRGP4 trap_CM_LoadMap
CALLV
pop
ADDRGP4 cg+20
CNSTI4 1
ASGNI4
ADDRGP4 CG_LoadTrailSystems
CALLV
pop
CNSTF4 1028443341
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 CG_LoadParticleSystems
CALLV
pop
CNSTF4 1028443341
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 CG_RegisterSounds
CALLV
pop
CNSTF4 1058642330
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 CG_RegisterGraphics
CALLV
pop
CNSTF4 1063675494
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 CG_InitWeapons
CALLV
pop
CNSTF4 1064514355
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 CG_InitUpgrades
CALLV
pop
CNSTF4 1065353216
ARGF4
ADDRGP4 CG_UpdateMediaFraction
CALLV
pop
ADDRGP4 CG_InitBuildables
CALLV
pop
ADDRLP4 48
ADDRGP4 BG_VoiceInit
CALLP4
ASGNP4
ADDRGP4 cgs+235700
ADDRLP4 48
INDIRP4
ASGNP4
ADDRGP4 cgs+235700
INDIRP4
ARGP4
ADDRGP4 cg_debugVoices+12
INDIRI4
ARGI4
ADDRGP4 BG_PrintVoices
CALLV
pop
ADDRGP4 CG_RegisterClients
CALLV
pop
ADDRGP4 cg+20
CNSTI4 0
ASGNI4
ADDRGP4 CG_InitMarkPolys
CALLV
pop
ADDRGP4 cg+112464
CNSTI1 0
ASGNI1
ADDRGP4 CG_SetConfigValues
CALLV
pop
ADDRGP4 CG_StartMusic
CALLV
pop
ADDRGP4 CG_ShaderStateChanged
CALLV
pop
CNSTI4 1
ARGI4
ADDRGP4 trap_S_ClearLoopingSounds
CALLV
pop
LABELV $1309
endproc CG_Init 52 12
export CG_Shutdown
proc CG_Shutdown 0 4
ADDRGP4 cgDC+244+61656
ARGP4
ADDRGP4 FreeFace
CALLV
pop
ADDRGP4 UIS_Shutdown
CALLV
pop
LABELV $1353
endproc CG_Shutdown 0 4
bss
align 1
LABELV $1357
skip 256
code
proc CG_VoIPString 300 20
ADDRGP4 $1358
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 256
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
ARGP4
ADDRGP4 $1361
ARGP4
ADDRLP4 256
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 256
INDIRI4
CNSTI4 0
NEI4 $1359
ADDRLP4 272
CNSTI4 0
ASGNI4
ADDRLP4 260
ADDRLP4 272
INDIRI4
ASGNI4
ADDRLP4 264
ADDRLP4 272
INDIRI4
ASGNI4
ADDRGP4 $1365
JUMPV
LABELV $1362
CNSTI4 1508
ADDRLP4 260
INDIRI4
MULI4
ADDRGP4 cgs+41460
ADDP4
INDIRI4
CNSTI4 0
EQI4 $1371
ADDRLP4 260
INDIRI4
ADDRGP4 cg+4
INDIRI4
NEI4 $1367
LABELV $1371
ADDRGP4 $1363
JUMPV
LABELV $1367
ADDRLP4 280
CNSTI4 1508
ASGNI4
ADDRLP4 280
INDIRI4
ADDRLP4 260
INDIRI4
MULI4
ADDRGP4 cgs+41460+68
ADDP4
INDIRI4
ADDRLP4 280
INDIRI4
ADDRGP4 cg+4
INDIRI4
MULI4
ADDRGP4 cgs+41460+68
ADDP4
INDIRI4
EQI4 $1372
ADDRGP4 $1363
JUMPV
LABELV $1372
ADDRLP4 264
INDIRI4
ADDRGP4 $1357
ADDP4
ARGP4
CNSTU4 256
ADDRLP4 264
INDIRI4
CVIU4 4
SUBU4
ARGU4
ADDRGP4 $1379
ARGP4
ADDRLP4 264
INDIRI4
CNSTI4 0
LEI4 $1382
ADDRLP4 284
ADDRGP4 $1380
ASGNP4
ADDRGP4 $1383
JUMPV
LABELV $1382
ADDRLP4 284
ADDRGP4 $226
ASGNP4
LABELV $1383
ADDRLP4 284
INDIRP4
ARGP4
ADDRLP4 260
INDIRI4
ARGI4
ADDRLP4 292
ADDRGP4 Q_snprintf
CALLI4
ASGNI4
ADDRLP4 268
ADDRLP4 292
INDIRI4
ASGNI4
ADDRLP4 264
INDIRI4
ADDRLP4 268
INDIRI4
ADDI4
CNSTI4 1
ADDI4
CVIU4 4
CNSTU4 256
LTU4 $1384
ADDRGP4 $1386
ARGP4
ADDRLP4 296
ADDRGP4 gettext
CALLP4
ASGNP4
ADDRLP4 296
INDIRP4
ARGP4
ADDRGP4 CG_Printf
CALLV
pop
ADDRGP4 $1364
JUMPV
LABELV $1384
ADDRLP4 264
ADDRLP4 264
INDIRI4
ADDRLP4 268
INDIRI4
ADDI4
ASGNI4
LABELV $1363
ADDRLP4 260
ADDRLP4 260
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
LABELV $1365
ADDRLP4 260
INDIRI4
ADDRGP4 cgs+31472
INDIRI4
LTI4 $1362
LABELV $1364
ADDRLP4 264
INDIRI4
ADDRGP4 $1357
ADDP4
CNSTI1 0
ASGNI1
ADDRGP4 $1360
JUMPV
LABELV $1359
ADDRLP4 0
ARGP4
ADDRGP4 $1389
ARGP4
ADDRLP4 260
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 260
INDIRI4
CNSTI4 0
NEI4 $1387
ADDRLP4 264
ADDRGP4 CG_CrosshairPlayer
CALLI4
ASGNI4
ADDRGP4 $1357
ARGP4
CNSTI4 256
ARGI4
ADDRGP4 $1164
ARGP4
ADDRLP4 264
INDIRI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRGP4 $1388
JUMPV
LABELV $1387
ADDRLP4 0
ARGP4
ADDRGP4 $1392
ARGP4
ADDRLP4 268
ADDRGP4 Q_stricmp
CALLI4
ASGNI4
ADDRLP4 268
INDIRI4
CNSTI4 0
NEI4 $1390
ADDRLP4 272
ADDRGP4 CG_LastAttacker
CALLI4
ASGNI4
ADDRGP4 $1357
ARGP4
CNSTI4 256
ARGI4
ADDRGP4 $1164
ARGP4
ADDRLP4 272
INDIRI4
ARGI4
ADDRGP4 Com_sprintf
CALLV
pop
ADDRGP4 $1391
JUMPV
LABELV $1390
CNSTP4 0
RETP4
ADDRGP4 $1356
JUMPV
LABELV $1391
LABELV $1388
LABELV $1360
ADDRGP4 $1357
RETP4
LABELV $1356
endproc CG_VoIPString 300 20
data
align 4
LABELV $1394
byte 4 0
export LoadFace
code
proc LoadFace 12 16
ADDRGP4 $1394
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1395
ADDRLP4 4
ADDRGP4 $1394
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1398
ADDRLP4 8
ADDRGP4 $1394
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1398
LABELV $1395
ADDRGP4 $1394
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
EQI4 $1400
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRI4
ARGI4
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
INDIRP4
ARGP4
ADDRGP4 trap_R_LoadFace
CALLV
pop
LABELV $1400
LABELV $1393
endproc LoadFace 12 16
data
align 4
LABELV $1403
byte 4 0
export FreeFace
code
proc FreeFace 12 12
ADDRGP4 $1403
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1404
ADDRLP4 4
ADDRGP4 $1403
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1406
ADDRLP4 8
ADDRGP4 $1403
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1406
LABELV $1404
ADDRGP4 $1403
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
EQI4 $1408
ADDRFP4 0
INDIRP4
ARGP4
ADDRGP4 trap_R_FreeFace
CALLV
pop
LABELV $1408
LABELV $1402
endproc FreeFace 12 12
data
align 4
LABELV $1411
byte 4 0
export LoadGlyph
code
proc LoadGlyph 12 16
ADDRGP4 $1411
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1412
ADDRLP4 4
ADDRGP4 $1411
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1414
ADDRLP4 8
ADDRGP4 $1411
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1414
LABELV $1412
ADDRGP4 $1411
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
EQI4 $1416
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
INDIRI4
ARGI4
ADDRFP4 12
INDIRP4
ARGP4
ADDRGP4 trap_R_LoadGlyph
CALLV
pop
LABELV $1416
LABELV $1410
endproc LoadGlyph 12 16
data
align 4
LABELV $1419
byte 4 0
export FreeGlyph
code
proc FreeGlyph 12 12
ADDRGP4 $1419
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1420
ADDRLP4 4
ADDRGP4 $1419
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1422
ADDRLP4 8
ADDRGP4 $1419
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1422
LABELV $1420
ADDRGP4 $1419
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
EQI4 $1424
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRI4
ARGI4
ADDRFP4 8
INDIRP4
ARGP4
ADDRGP4 trap_R_FreeGlyph
CALLV
pop
LABELV $1424
LABELV $1418
endproc FreeGlyph 12 12
data
align 4
LABELV $1427
byte 4 0
export Glyph
code
proc Glyph 12 16
ADDRGP4 $1427
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1428
ADDRLP4 4
ADDRGP4 $1427
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1430
ADDRLP4 8
ADDRGP4 $1427
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1430
LABELV $1428
ADDRGP4 $1427
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
EQI4 $1432
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
INDIRP4
ARGP4
ADDRFP4 12
INDIRP4
ARGP4
ADDRGP4 trap_R_Glyph
CALLV
pop
LABELV $1432
LABELV $1426
endproc Glyph 12 16
data
align 4
LABELV $1435
byte 4 0
export FreeCachedGlyphs
code
proc FreeCachedGlyphs 12 12
ADDRGP4 $1435
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1436
ADDRLP4 4
ADDRGP4 $1435
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1438
ADDRLP4 8
ADDRGP4 $1435
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1438
LABELV $1436
ADDRGP4 $1435
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
EQI4 $1440
ADDRFP4 0
INDIRP4
ARGP4
ADDRGP4 trap_R_FreeCachedGlyphs
CALLV
pop
LABELV $1440
LABELV $1434
endproc FreeCachedGlyphs 12 12
data
align 4
LABELV $1443
byte 4 0
export Gettext
code
proc Gettext 12 12
ADDRGP4 $1443
INDIRI4
CNSTI4 1
BANDI4
CNSTI4 0
NEI4 $1444
ADDRLP4 4
ADDRGP4 $1443
ASGNP4
ADDRLP4 4
INDIRP4
ADDRLP4 4
INDIRP4
INDIRI4
CNSTI4 1
BORI4
ASGNI4
ADDRGP4 $1397
ARGP4
ADDRLP4 0
ARGP4
CNSTI4 2
ARGI4
ADDRGP4 trap_Cvar_VariableStringBuffer
CALLV
pop
ADDRLP4 0
INDIRI1
CVII4 1
CNSTI4 49
NEI4 $1446
ADDRLP4 8
ADDRGP4 $1443
ASGNP4
ADDRLP4 8
INDIRP4
ADDRLP4 8
INDIRP4
INDIRI4
CNSTI4 2
BORI4
ASGNI4
LABELV $1446
LABELV $1444
ADDRGP4 $1443
INDIRI4
CNSTI4 2
BANDI4
CNSTI4 0
NEI4 $1448
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
INDIRI4
ARGI4
ADDRGP4 strncpy
CALLP4
pop
ADDRGP4 $1449
JUMPV
LABELV $1448
ADDRFP4 0
INDIRP4
ARGP4
ADDRFP4 4
INDIRP4
ARGP4
ADDRFP4 8
INDIRI4
ARGI4
ADDRGP4 trap_Gettext
CALLV
pop
LABELV $1449
LABELV $1442
endproc Gettext 12 12
bss
align 1
LABELV $1451
skip 256000
data
align 4
LABELV $1452
byte 4 0
export gettext
code
proc gettext 12 12
ADDRLP4 8
ADDRGP4 $1452
ASGNP4
ADDRLP4 4
ADDRLP4 8
INDIRP4
INDIRI4
ASGNI4
ADDRLP4 8
INDIRP4
ADDRLP4 4
INDIRI4
CNSTI4 1
ADDI4
ASGNI4
ADDRLP4 0
CNSTI4 32000
ADDRLP4 4
INDIRI4
CNSTI4 7
BANDI4
MULI4
ADDRGP4 $1451
ADDP4
ASGNP4
ADDRLP4 0
INDIRP4
ARGP4
ADDRFP4 0
INDIRP4
ARGP4
CNSTI4 32000
ARGI4
ADDRGP4 Gettext
CALLV
pop
ADDRLP4 0
INDIRP4
RETP4
LABELV $1450
endproc gettext 12 12
import Q_snprintf
import Q_vsnprintf
bss
export cg_noPrintDuplicate
align 4
LABELV cg_noPrintDuplicate
skip 272
import trap_Gettext
import trap_GetDemoName
import trap_GetDemoPos
import trap_GetDemoState
import trap_GetEntityToken
import trap_getCameraInfo
import trap_startCamera
import trap_loadCamera
import trap_RealTime
import trap_SnapVector
import trap_CIN_SetExtents
import trap_CIN_DrawCinematic
import trap_CIN_RunCinematic
import trap_CIN_StopCinematic
import trap_CIN_PlayCinematic
import trap_Key_GetOverstrikeMode
import trap_Key_SetOverstrikeMode
import trap_Key_SetBinding
import trap_Key_GetBindingBuf
import trap_Key_KeynumToStringBuf
import trap_Key_GetKey
import trap_Key_SetCatcher
import trap_Key_GetCatcher
import trap_Key_IsDown
import trap_R_FreeCachedGlyphs
import trap_R_Glyph
import trap_R_FreeGlyph
import trap_R_LoadGlyph
import trap_R_FreeFace
import trap_R_LoadFace
import trap_R_RegisterFont
import trap_MemoryRemaining
import testPrintFloat
import testPrintInt
import trap_SetUserCmdValue
import trap_GetUserCmd
import trap_GetCurrentCmdNumber
import trap_GetServerCommand
import trap_GetSnapshot
import trap_GetCurrentSnapshotNumber
import trap_GetGameState
import trap_GetGlconfig
import trap_R_RemapShader
import trap_R_LerpTag
import trap_R_ModelBounds
import trap_R_DrawStretchPic
import trap_R_SetColor
import trap_R_RenderScene
import trap_R_LightForPoint
import trap_R_AddAdditiveLightToScene
import trap_R_AddLightToScene
import trap_R_inPVS
import trap_R_AddPolysToScene
import trap_R_AddPolyToScene
import trap_R_AddRefEntityToScene
import trap_R_ClearScene
import trap_R_RegisterShaderNoMip
import trap_R_RegisterShader
import trap_R_RegisterSkin
import trap_R_RegisterModel
import trap_R_LoadWorldMap
import trap_S_StopBackgroundTrack
import trap_S_StartBackgroundTrack
import trap_S_RegisterSound
import trap_S_Respatialize
import trap_S_UpdateEntityPosition
import trap_S_AddRealLoopingSound
import trap_S_AddLoopingSound
import trap_S_ClearLoopingSounds
import trap_S_StartLocalSound
import trap_S_StopLoopingSound
import trap_S_StartSound
import trap_CM_MarkFragments
import trap_CM_TransformedBiSphereTrace
import trap_CM_BiSphereTrace
import trap_CM_TransformedCapsuleTrace
import trap_CM_CapsuleTrace
import trap_CM_TransformedBoxTrace
import trap_CM_BoxTrace
import trap_CM_TransformedPointContents
import trap_CM_PointContents
import trap_CM_TempBoxModel
import trap_CM_InlineModel
import trap_CM_NumInlineModels
import trap_CM_LoadMap
import trap_UpdateScreen
import trap_SendClientCommand
import trap_AddCommand
import trap_SendConsoleCommand
import trap_FS_GetFileList
import trap_FS_Seek
import trap_FS_FCloseFile
import trap_FS_Write
import trap_FS_Read
import trap_FS_FOpenFile
import trap_LiteralArgs
import trap_Args
import trap_Argv
import trap_Argc
import trap_Cvar_VariableStringBuffer
import trap_Cvar_Set
import trap_Cvar_Update
import trap_Cvar_Register
import trap_Milliseconds
import trap_Error
import trap_Print
import CG_TutorialText
import CG_WritePTRCode
import CG_ReadPTRCode
import CG_DestroyTestTS_f
import CG_TestTS_f
import CG_AddTrails
import CG_IsTrailSystemValid
import CG_DestroyTrailSystem
import CG_SpawnNewTrailSystem
import CG_RegisterTrailSystem
import CG_LoadTrailSystems
import CG_DestroyTestPS_f
import CG_TestPS_f
import CG_ParticleSystemEntity
import CG_AddParticles
import CG_SetParticleSystemNormal
import CG_IsParticleSystemValid
import CG_IsParticleSystemInfinite
import CG_DestroyParticleSystem
import CG_SpawnNewParticleSystem
import CG_RegisterParticleSystem
import CG_LoadParticleSystems
import CG_SetAttachmentOffset
import CG_SetAttachmentParticle
import CG_SetAttachmentTag
import CG_SetAttachmentCent
import CG_SetAttachmentPoint
import CG_AttachToParticle
import CG_AttachToTag
import CG_AttachToCent
import CG_AttachToPoint
import CG_Attached
import CG_AttachmentCentNum
import CG_AttachmentVelocity
import CG_AttachmentAxis
import CG_AttachmentDir
import CG_AttachmentPoint
import CG_CheckChangedPredictableEvents
import CG_TransitionPlayerState
import CG_Respawn
import CG_ShaderStateChanged
import CG_SetConfigValues
import CG_ParseServerinfo
import CG_ExecuteNewServerCommands
import CG_RequestScores
import CG_InitConsoleCommands
import CG_ConsoleCommand
import CG_ProcessSnapshots
import CG_ImpactMark
import CG_AddMarks
import CG_InitMarkPolys
import CG_AlienSense
import CG_Scanner
import CG_UpdateEntityPositions
import CG_DrawItemSelectText
import CG_DrawItemSelect
import CG_AddPlayerWeapon
import CG_AddViewWeapon
import CG_ShotgunFire
import CG_Bullet
import CG_MissileHitEntity
import CG_MissileHitWall
import CG_FireWeapon
import CG_RegisterWeapon
import CG_InitWeapons
import CG_RegisterUpgrade
import CG_InitUpgrades
import CG_Weapon_f
import CG_PrevWeapon_f
import CG_NextWeapon_f
import CG_LinkLocation
import CG_PositionRotatedEntityOnTag
import CG_PositionEntityOnTag
import CG_AdjustPositionForMover
import CG_Beam
import CG_AddPacketEntities
import CG_SetEntitySoundPosition
import CG_DrawBoundingBox
import CG_PainEvent
import CG_EntityEvent
import CG_CheckEvents
import CG_PredictPlayerState
import CG_BiSphereTrace
import CG_CapTrace
import CG_Trace
import CG_PointContents
import CG_BuildSolidList
import CG_ModelDoor
import CG_AnimMapObj
import CG_RunLerpFrame
import CG_AlienBuildableExplosion
import CG_HumanBuildableExplosion
import CG_InitBuildables
import CG_DrawDominationStatus
import CG_DrawBuildableStatus
import CG_BuildableStatusParse
import CG_Buildable
import CG_GhostBuildable
import CG_GetPlayerLocation
import CG_Bleed
import CG_PlayerDisconnect
import CG_CustomSound
import CG_PrecacheClientInfo
import CG_NewClientInfo
import CG_ResetPlayerEntity
import CG_Corpse
import CG_Player
import CG_DrawField
import CG_ResetPainBlend
import CG_UpdateMediaFraction
import CG_DrawLoadingScreen
import CG_Text_PaintChar
import CG_GetKillerText
import CG_GetTeamColor
import CG_SetPrintString
import CG_RunMenuScript
import CG_GetValue
import CG_OwnerDraw
import CG_DrawActive
import CG_CenterPrint
import CG_AddSpeed
import CG_AddLagometerSnapshotInfo
import CG_AddLagometerFrameInfo
import CG_GetColorCharForHealth
import CG_KeyBinding
import CG_WorldToScreen
import CG_DrawTopBottom
import CG_DrawSides
import CG_DrawRect
import CG_ColorForHealth
import CG_TileClear
import CG_FadeColor
import CG_DrawStrlen
import CG_ClearClipRegion
import CG_SetClipRegion
import CG_DrawFadePic
import CG_DrawPic
import CG_FillRect
import CG_AdjustFrom640
import CG_DrawPlane
import CG_OffsetShoulderView
import CG_OffsetThirdPersonView
import CG_OffsetFirstPersonView
import CG_DrawActiveFrame
import CG_AddBufferedSound
import CG_TestModelPrevSkin_f
import CG_TestModelNextSkin_f
import CG_TestModelPrevFrame_f
import CG_TestModelNextFrame_f
import CG_TestGun_f
import CG_TestModel_f
import CG_addSmoothOp
import CG_EventHandling
import CG_MouseEvent
import CG_KeyEvent
export cg_translateCenterPrint
align 4
LABELV cg_translateCenterPrint
skip 272
export cg_chatTeamPrefix
align 4
LABELV cg_chatTeamPrefix
skip 272
export cg_emoticons
align 4
LABELV cg_emoticons
skip 272
export cg_voice
align 4
LABELV cg_voice
skip 272
export cg_projectileNudge
align 4
LABELV cg_projectileNudge
skip 272
export cg_optimizePrediction
align 4
LABELV cg_optimizePrediction
skip 272
export cg_debugRandom
align 4
LABELV cg_debugRandom
skip 272
export ui_humanTeamVoteActive
align 4
LABELV ui_humanTeamVoteActive
skip 272
export ui_alienTeamVoteActive
align 4
LABELV ui_alienTeamVoteActive
skip 272
export ui_voteActive
align 4
LABELV ui_voteActive
skip 272
export ui_dialog
align 4
LABELV ui_dialog
skip 272
export ui_stages
align 4
LABELV ui_stages
skip 272
export ui_carriage
align 4
LABELV ui_carriage
skip 272
export ui_currentClass
align 4
LABELV ui_currentClass
skip 272
export cg_debugVoices
align 4
LABELV cg_debugVoices
skip 272
export cg_unlagged
align 4
LABELV cg_unlagged
skip 272
export cg_sprintToggle
align 4
LABELV cg_sprintToggle
skip 272
export cg_stickySpec
align 4
LABELV cg_stickySpec
skip 272
export cg_painBlendZoom
align 4
LABELV cg_painBlendZoom
skip 272
export cg_painBlendScale
align 4
LABELV cg_painBlendScale
skip 272
export cg_painBlendMax
align 4
LABELV cg_painBlendMax
skip 272
export cg_painBlendDownRate
align 4
LABELV cg_painBlendDownRate
skip 272
export cg_painBlendUpRate
align 4
LABELV cg_painBlendUpRate
skip 272
export cg_tutorial
align 4
LABELV cg_tutorial
skip 272
export cg_disableScannerPlane
align 4
LABELV cg_disableScannerPlane
skip 272
export cg_disableCommandDialogs
align 4
LABELV cg_disableCommandDialogs
skip 272
export cg_disableBuildDialogs
align 4
LABELV cg_disableBuildDialogs
skip 272
export cg_disableUpgradeDialogs
align 4
LABELV cg_disableUpgradeDialogs
skip 272
export cg_disableWarningDialogs
align 4
LABELV cg_disableWarningDialogs
skip 272
export cg_debugPVS
align 4
LABELV cg_debugPVS
skip 272
export cg_debugTrails
align 4
LABELV cg_debugTrails
skip 272
export cg_debugParticles
align 4
LABELV cg_debugParticles
skip 272
export cg_lightFlare
align 4
LABELV cg_lightFlare
skip 272
export cg_consoleLatency
align 4
LABELV cg_consoleLatency
skip 272
export cg_bounceParticles
align 4
LABELV cg_bounceParticles
skip 272
export cg_depthSortParticles
align 4
LABELV cg_depthSortParticles
skip 272
export cg_disableBlueprintErrors
align 4
LABELV cg_disableBlueprintErrors
skip 272
export cg_wwSmoothTime
align 4
LABELV cg_wwSmoothTime
skip 272
export cg_drawBBOX
align 4
LABELV cg_drawBBOX
skip 272
export cg_drawSurfNormal
align 4
LABELV cg_drawSurfNormal
skip 272
export cg_noTaunt
align 4
LABELV cg_noTaunt
skip 272
export cg_timescale
align 4
LABELV cg_timescale
skip 272
export cg_timescaleFadeSpeed
align 4
LABELV cg_timescaleFadeSpeed
skip 272
export cg_timescaleFadeEnd
align 4
LABELV cg_timescaleFadeEnd
skip 272
export cg_cameraMode
align 4
LABELV cg_cameraMode
skip 272
export pmove_msec
align 4
LABELV pmove_msec
skip 272
export pmove_fixed
align 4
LABELV pmove_fixed
skip 272
export cg_smoothClients
align 4
LABELV cg_smoothClients
skip 272
export cg_hudFiles
align 4
LABELV cg_hudFiles
skip 272
export cg_noVoiceText
align 4
LABELV cg_noVoiceText
skip 272
export cg_noVoiceChats
align 4
LABELV cg_noVoiceChats
skip 272
export cg_teamChatsOnly
align 4
LABELV cg_teamChatsOnly
skip 272
export cg_blood
align 4
LABELV cg_blood
skip 272
export cg_paused
align 4
LABELV cg_paused
skip 272
export cg_stats
align 4
LABELV cg_stats
skip 272
export cg_synchronousClients
align 4
LABELV cg_synchronousClients
skip 272
export cg_drawSpeed
align 4
LABELV cg_drawSpeed
skip 272
export cg_lagometer
align 4
LABELV cg_lagometer
skip 272
export cg_stereoSeparation
align 4
LABELV cg_stereoSeparation
skip 272
export cg_thirdPersonRange
align 4
LABELV cg_thirdPersonRange
skip 272
export cg_thirdPersonPitchFollow
align 4
LABELV cg_thirdPersonPitchFollow
skip 272
export cg_thirdPersonShoulderViewMode
align 4
LABELV cg_thirdPersonShoulderViewMode
skip 272
export cg_thirdPersonAngle
align 4
LABELV cg_thirdPersonAngle
skip 272
export cg_thirdPerson
align 4
LABELV cg_thirdPerson
skip 272
export cg_tracerLength
align 4
LABELV cg_tracerLength
skip 272
export cg_tracerWidth
align 4
LABELV cg_tracerWidth
skip 272
export cg_tracerChance
align 4
LABELV cg_tracerChance
skip 272
export cg_gun_z
align 4
LABELV cg_gun_z
skip 272
export cg_gun_y
align 4
LABELV cg_gun_y
skip 272
export cg_gun_x
align 4
LABELV cg_gun_x
skip 272
export cg_gun_frame
align 4
LABELV cg_gun_frame
skip 272
export cg_drawGun
align 4
LABELV cg_drawGun
skip 272
export cg_viewsize
align 4
LABELV cg_viewsize
skip 272
export cg_addMarks
align 4
LABELV cg_addMarks
skip 272
export cg_footsteps
align 4
LABELV cg_footsteps
skip 272
export cg_showmiss
align 4
LABELV cg_showmiss
skip 272
export cg_noPlayerAnims
align 4
LABELV cg_noPlayerAnims
skip 272
export cg_debugMove
align 4
LABELV cg_debugMove
skip 272
export cg_nopredict
align 4
LABELV cg_nopredict
skip 272
export cg_errorDecay
align 4
LABELV cg_errorDecay
skip 272
export cg_debugEvents
align 4
LABELV cg_debugEvents
skip 272
export cg_debugPosition
align 4
LABELV cg_debugPosition
skip 272
export cg_debugAnim
align 4
LABELV cg_debugAnim
skip 272
export cg_animSpeed
align 4
LABELV cg_animSpeed
skip 272
export cg_draw2D
align 4
LABELV cg_draw2D
skip 272
export cg_teamOverlayUserinfo
align 4
LABELV cg_teamOverlayUserinfo
skip 272
export cg_teamOverlayMaxPlayers
align 4
LABELV cg_teamOverlayMaxPlayers
skip 272
export cg_drawTeamOverlay
align 4
LABELV cg_drawTeamOverlay
skip 272
export cg_crosshairSize
align 4
LABELV cg_crosshairSize
skip 272
export cg_drawCrosshairNames
align 4
LABELV cg_drawCrosshairNames
skip 272
export cg_drawCrosshair
align 4
LABELV cg_drawCrosshair
skip 272
export cg_drawChargeBar
align 4
LABELV cg_drawChargeBar
skip 272
export cg_drawSnapshot
align 4
LABELV cg_drawSnapshot
skip 272
export cg_drawDemoState
align 4
LABELV cg_drawDemoState
skip 272
export cg_drawFPS
align 4
LABELV cg_drawFPS
skip 272
export cg_drawClock
align 4
LABELV cg_drawClock
skip 272
export cg_drawTimer
align 4
LABELV cg_drawTimer
skip 272
export cg_shadows
align 4
LABELV cg_shadows
skip 272
export cg_swingSpeed
align 4
LABELV cg_swingSpeed
skip 272
export cg_runroll
align 4
LABELV cg_runroll
skip 272
export cg_runpitch
align 4
LABELV cg_runpitch
skip 272
export cg_centertime
align 4
LABELV cg_centertime
skip 272
export cg_teslaTrailTime
align 4
LABELV cg_teslaTrailTime
skip 272
import cg_markPolys
export cg_buildables
align 4
LABELV cg_buildables
skip 12320
export cg_upgrades
align 4
LABELV cg_upgrades
skip 384
export cg_weapons
align 4
LABELV cg_weapons
skip 41600
export cgDC
align 4
LABELV cgDC
skip 79752
export cg_entities
align 4
LABELV cg_entities
skip 1888256
export cg
align 4
LABELV cg
skip 155320
export cgs
align 4
LABELV cgs
skip 235712
import g_nameBind2
import g_nameBind1
import BindingFromName
import trap_Parse_SourceFileAndLine
import trap_Parse_ReadToken
import trap_Parse_FreeSource
import trap_Parse_LoadSource
import trap_Parse_AddGlobalDefine
import UI_EscapeEmoticons
import UI_Text_IsEmoticon
import UI_Text_EmHeight
import UI_Text_EmWidth
import UI_Text_Height
import UI_Text_Width
import UI_Text_Paint_Limit
import UI_Text_Paint
import UI_DrawTextBlock
import Item_Text_Wrap
import Item_Text_Wrapped_Paint
import UI_Glyph
import trap_R_SetClipRegion
import Controls_SetDefaults
import Controls_SetConfig
import Controls_GetConfig
import UI_OutOfMemory
import UI_InitMemory
import UI_Alloc
import UI_RemoveCaptureFunc
import UI_InstallCaptureFunc
import Display_CacheAll
import Menu_SetFeederSelection
import Menu_Paint
import Menu_Update
import Menus_CloseAll
import LerpColor
import Display_HandleKey
import Menus_CloseByName
import Menus_FindByName
import Display_KeyBindPending
import Display_CursorType
import Display_MouseMove
import Display_CaptureItem
import Display_GetContext
import Menus_ReplaceActive
import Menus_Activate
import Menus_AnyFullScreenVisible
import Menu_Reset
import Menus_ReplaceActiveByName
import Menus_ActivateByName
import Menu_PaintAll
import Menu_UpdateAll
import Menu_New
import Menu_Count
import PC_Script_Parse
import PC_String_Parse
import PC_Rect_Parse
import PC_Int_Parse
import PC_Color_Parse
import PC_Float_Parse
import Script_Parse
import String_Parse
import Rect_Parse
import Int_Parse
import Color_Parse
import Float_Parse
import Menu_ScrollFeeder
import Menu_HandleMouseMove
import Menu_HandleKey
import Menu_GetFocused
import Menu_PostParse
import Item_Init
import Menu_Init
import Display_ExpandMacros
import Init_Display
import String_Report
import String_Init
import String_Alloc
import UIS_Shutdown
import cmdcmp
import BG_TeamName
import BG_LoadEmoticons
import BG_VoiceTrackFind
import BG_VoiceTrackByNum
import BG_VoiceCmdByNum
import BG_VoiceCmdFind
import BG_VoiceByName
import BG_PrintVoices
import BG_VoiceInit
import BG_PrimaryWeapon
import BG_BuildableIsAllowed
import BG_ClassIsAllowed
import BG_UpgradeIsAllowed
import BG_WeaponIsAllowed
import BG_InitAllowedGameElements
import BG_ParseCSVBuildableList
import BG_ParseCSVClassList
import BG_ParseCSVEquipmentList
import atoi_neg
import atof_neg
import BG_PlayerTouchesItem
import BG_PlayerStateToEntityStateExtraPolate
import BG_PlayerStateToEntityState
import BG_AddPredictableEventToPlayerstate
import BG_EvaluateTrajectoryDelta
import BG_EvaluateTrajectory
import BG_MemoryInfo
import BG_DefragmentMemory
import BG_Free
import BG_InitMemory
import BG_Alloc
import BG_UpgradeAllowedInStage
import BG_Upgrade
import BG_UpgradeByName
import BG_WeaponAllowedInStage
import BG_Weapon
import BG_WeaponByName
import BG_InitClassConfigs
import BG_AlienCanEvolve
import BG_ClassCanEvolveFromTo
import BG_ClassHasAbility
import BG_ClassBoundingBox
import BG_ClassConfig
import BG_ClassAllowedInStage
import BG_Class
import BG_ClassByName
import BG_InitBuildableConfigs
import BG_BuildableBoundingBox
import BG_BuildableConfig
import BG_BuildableAllowedInStage
import BG_Buildable
import BG_BuildableByEntityName
import BG_BuildableByName
import BG_UnpackEntityNumbers
import BG_PackEntityNumbers
import BG_HasEnergyWeapon
import BG_GetPlayerWeapon
import BG_PlayerPoisonCloudTime
import BG_PlayerCanChangeWeapon
import BG_GetValueOfPlayer
import BG_PositionBuildableRelativeToPlayer
import BG_GetClientNormal
import BG_RotateAxis
import BG_UpgradeIsActive
import BG_DeactivateUpgrade
import BG_ActivateUpgrade
import BG_InventoryContainsUpgrade
import BG_RemoveUpgradeFromInventory
import BG_AddUpgradeToInventory
import BG_SlotsForInventory
import BG_InventoryContainsWeapon
import BG_WeaponIsFull
import BG_EventName
import Pmove
import PM_UpdateViewAngles
import Info_NextPair
import Info_Validate
import Info_SetValueForKey_Big
import Info_SetValueForKey
import Info_RemoveKey_big
import Info_RemoveKey
import Info_ValueForKey
import Com_TruncateLongString
import va
import Q_UTF8Unstore
import Q_UTF8Store
import Q_UTF8Encode
import Q_UTF8CodePoint
import Q_UTF8ContByte
import Q_UTF8PrintStrlen
import Q_UTF8Strlen
import Q_UTF8Width
import Q_CountChar
import Q_ParseNewlines
import Q_CleanStr
import Q_PrintStrlen
import Q_strcat
import Q_strncpyz
import Q_stristr
import Q_strrchr
import Q_strupr
import Q_strlwr
import Q_stricmpn
import Q_strncmp
import Q_stricmp
import Q_isintegral
import Q_isanumber
import Q_isalpha
import Q_isupper
import Q_islower
import Q_isprint
import Com_ClientListParse
import Com_ClientListString
import Com_ClientListRemove
import Com_ClientListAdd
import Com_ClientListContains
import Com_RandomBytes
import Com_SkipCharset
import Com_SkipTokens
import Com_sprintf
import Com_HexStrToInt
import Parse3DMatrix
import Parse2DMatrix
import Parse1DMatrix
import SkipRestOfLine
import SkipBracedSection
import COM_MatchToken
import COM_ParseWarning
import COM_ParseError
import COM_Compress
import COM_ParseExt
import COM_Parse
import COM_GetCurrentParseLine
import COM_BeginParseSession
import COM_DefaultExtension
import COM_StripExtension
import COM_GetExtension
import COM_SkipPath
import Com_Clamp
import DistanceBetweenLineSegments
import DistanceBetweenLineSegmentsSquared
import VectorMaxComponent
import VectorMinComponent
import pointToLineDistance
import VectorDistance
import ProjectPointOntoVector
import GetPerpendicularViewVector
import Q_isnan
import PerpendicularVector
import AngleVectors
import VectorMatrixMultiply
import MatrixMultiply
import MakeNormalVectors
import RotateAroundDirection
import RotatePointAroundVector
import ProjectPointOnPlane
import PlaneFromPoints
import AngleDelta
import AngleNormalize180
import AngleNormalize360
import AnglesSubtract
import AngleSubtract
import LerpAngle
import AngleMod
import BoundsIntersectPoint
import BoundsIntersectSphere
import BoundsIntersect
import BoxOnPlaneSide
import SetPlaneSignbits
import AxisCopy
import AxisClear
import AxisToAngles
import AnglesToAxis
import vectoangles
import Q_crandom
import Q_random
import Q_rand
import Q_acos
import Q_log2
import VectorRotate
import Vector4Scale
import VectorNormalize2
import VectorNormalize
import CrossProduct
import VectorInverse
import VectorNormalizeFast
import DistanceSquared
import Distance
import VectorLengthSquared
import VectorLength
import VectorCompare
import AddPointToBounds
import ClearBounds
import RadiusFromBounds
import NormalizeColor
import ColorBytes4
import ColorBytes3
import _VectorMA
import _VectorScale
import _VectorCopy
import _VectorAdd
import _VectorSubtract
import _DotProduct
import ByteToDir
import DirToByte
import ClampShort
import ClampChar
import Q_rsqrt
import Q_fabs
import axisDefault
import vec3_origin
import g_color_table
import Q_StripIndentMarker
import colorDkGrey
import colorMdGrey
import colorLtGrey
import colorWhite
import colorCyan
import colorMagenta
import colorYellow
import colorBlue
import colorGreen
import colorRed
import colorBlack
import bytedirs
import Hunk_Alloc
import FloatSwap
import LongSwap
import ShortSwap
import rint
import pow
import acos
import fabs
import abs
import tan
import atan2
import cos
import sin
import sqrt
import floor
import ceil
import memcpy
import memset
import memmove
import sscanf
import strtol
import _atoi
import atoi
import strtod
import _atof
import atof
import toupper
import tolower
import strncpy
import strstr
import strrchr
import strchr
import strcmp
import strcpy
import strcat
import strlen
import bsearch
import rand
import srand
import qsort
lit
align 1
LABELV $1397
byte 1 92
byte 1 73
byte 1 83
byte 1 95
byte 1 71
byte 1 69
byte 1 84
byte 1 84
byte 1 69
byte 1 88
byte 1 84
byte 1 95
byte 1 83
byte 1 85
byte 1 80
byte 1 80
byte 1 79
byte 1 82
byte 1 84
byte 1 69
byte 1 68
byte 1 0
align 1
LABELV $1392
byte 1 97
byte 1 116
byte 1 116
byte 1 97
byte 1 99
byte 1 107
byte 1 101
byte 1 114
byte 1 0
align 1
LABELV $1389
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 104
byte 1 97
byte 1 105
byte 1 114
byte 1 0
align 1
LABELV $1386
byte 1 94
byte 1 51
byte 1 87
byte 1 65
byte 1 82
byte 1 78
byte 1 73
byte 1 78
byte 1 71
byte 1 58
byte 1 32
byte 1 118
byte 1 111
byte 1 105
byte 1 112
byte 1 83
byte 1 116
byte 1 114
byte 1 105
byte 1 110
byte 1 103
byte 1 32
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 102
byte 1 108
byte 1 111
byte 1 119
byte 1 101
byte 1 100
byte 1 10
byte 1 0
align 1
LABELV $1380
byte 1 44
byte 1 0
align 1
LABELV $1379
byte 1 37
byte 1 115
byte 1 37
byte 1 100
byte 1 0
align 1
LABELV $1361
byte 1 116
byte 1 101
byte 1 97
byte 1 109
byte 1 0
align 1
LABELV $1358
byte 1 99
byte 1 108
byte 1 95
byte 1 118
byte 1 111
byte 1 105
byte 1 112
byte 1 83
byte 1 101
byte 1 110
byte 1 100
byte 1 84
byte 1 97
byte 1 114
byte 1 103
byte 1 101
byte 1 116
byte 1 0
align 1
LABELV $1344
byte 1 67
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 116
byte 1 47
byte 1 83
byte 1 101
byte 1 114
byte 1 118
byte 1 101
byte 1 114
byte 1 32
byte 1 103
byte 1 97
byte 1 109
byte 1 101
byte 1 32
byte 1 109
byte 1 105
byte 1 115
byte 1 109
byte 1 97
byte 1 116
byte 1 99
byte 1 104
byte 1 58
byte 1 32
byte 1 37
byte 1 115
byte 1 47
byte 1 37
byte 1 115
byte 1 0
align 1
LABELV $1343
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 0
align 1
LABELV $1329
byte 1 111
byte 1 117
byte 1 116
byte 1 108
byte 1 105
byte 1 110
byte 1 101
byte 1 0
align 1
LABELV $1326
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 98
byte 1 105
byte 1 103
byte 1 99
byte 1 104
byte 1 97
byte 1 114
byte 1 115
byte 1 0
align 1
LABELV $1324
byte 1 119
byte 1 104
byte 1 105
byte 1 116
byte 1 101
byte 1 0
align 1
LABELV $1303
byte 1 101
byte 1 109
byte 1 111
byte 1 116
byte 1 105
byte 1 99
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 37
byte 1 115
byte 1 95
byte 1 37
byte 1 100
byte 1 120
byte 1 49
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1284
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 108
byte 1 105
byte 1 100
byte 1 101
byte 1 114
byte 1 98
byte 1 117
byte 1 116
byte 1 116
byte 1 95
byte 1 49
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1281
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 108
byte 1 105
byte 1 100
byte 1 101
byte 1 114
byte 1 50
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1278
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 99
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 116
byte 1 104
byte 1 117
byte 1 109
byte 1 98
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1275
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 99
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 97
byte 1 114
byte 1 114
byte 1 111
byte 1 119
byte 1 95
byte 1 114
byte 1 105
byte 1 103
byte 1 104
byte 1 116
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1272
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 99
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 97
byte 1 114
byte 1 114
byte 1 111
byte 1 119
byte 1 95
byte 1 108
byte 1 101
byte 1 102
byte 1 116
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1269
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 99
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 97
byte 1 114
byte 1 114
byte 1 111
byte 1 119
byte 1 95
byte 1 117
byte 1 112
byte 1 95
byte 1 97
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1266
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 99
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 97
byte 1 114
byte 1 114
byte 1 111
byte 1 119
byte 1 95
byte 1 100
byte 1 119
byte 1 110
byte 1 95
byte 1 97
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1263
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 115
byte 1 99
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 98
byte 1 97
byte 1 114
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1260
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 105
byte 1 101
byte 1 110
byte 1 116
byte 1 98
byte 1 97
byte 1 114
byte 1 50
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $1166
byte 1 37
byte 1 52
byte 1 100
byte 1 0
align 1
LABELV $1164
byte 1 37
byte 1 100
byte 1 0
align 1
LABELV $1161
byte 1 82
byte 1 101
byte 1 97
byte 1 100
byte 1 121
byte 1 0
align 1
LABELV $1046
byte 1 85
byte 1 73
byte 1 32
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 32
byte 1 108
byte 1 111
byte 1 97
byte 1 100
byte 1 32
byte 1 116
byte 1 105
byte 1 109
byte 1 101
byte 1 32
byte 1 61
byte 1 32
byte 1 37
byte 1 100
byte 1 32
byte 1 109
byte 1 105
byte 1 108
byte 1 108
byte 1 105
byte 1 32
byte 1 115
byte 1 101
byte 1 99
byte 1 111
byte 1 110
byte 1 100
byte 1 115
byte 1 10
byte 1 0
align 1
LABELV $1043
byte 1 108
byte 1 111
byte 1 97
byte 1 100
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 0
align 1
LABELV $1029
byte 1 94
byte 1 49
byte 1 100
byte 1 101
byte 1 102
byte 1 97
byte 1 117
byte 1 108
byte 1 116
byte 1 32
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 32
byte 1 102
byte 1 105
byte 1 108
byte 1 101
byte 1 32
byte 1 110
byte 1 111
byte 1 116
byte 1 32
byte 1 102
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 58
byte 1 32
byte 1 117
byte 1 105
byte 1 47
byte 1 104
byte 1 117
byte 1 100
byte 1 46
byte 1 116
byte 1 120
byte 1 116
byte 1 44
byte 1 32
byte 1 117
byte 1 110
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 32
byte 1 116
byte 1 111
byte 1 32
byte 1 99
byte 1 111
byte 1 110
byte 1 116
byte 1 105
byte 1 110
byte 1 117
byte 1 101
byte 1 33
byte 1 10
byte 1 0
align 1
LABELV $1026
byte 1 94
byte 1 51
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 32
byte 1 102
byte 1 105
byte 1 108
byte 1 101
byte 1 32
byte 1 110
byte 1 111
byte 1 116
byte 1 32
byte 1 102
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 58
byte 1 32
byte 1 37
byte 1 115
byte 1 44
byte 1 32
byte 1 117
byte 1 115
byte 1 105
byte 1 110
byte 1 103
byte 1 32
byte 1 100
byte 1 101
byte 1 102
byte 1 97
byte 1 117
byte 1 108
byte 1 116
byte 1 10
byte 1 0
align 1
LABELV $1010
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 100
byte 1 101
byte 1 102
byte 1 0
align 1
LABELV $1004
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 71
byte 1 108
byte 1 111
byte 1 98
byte 1 97
byte 1 108
byte 1 68
byte 1 101
byte 1 102
byte 1 0
align 1
LABELV $990
byte 1 117
byte 1 105
byte 1 47
byte 1 116
byte 1 101
byte 1 115
byte 1 116
byte 1 104
byte 1 117
byte 1 100
byte 1 46
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 0
align 1
LABELV $977
byte 1 115
byte 1 104
byte 1 97
byte 1 100
byte 1 111
byte 1 119
byte 1 67
byte 1 111
byte 1 108
byte 1 111
byte 1 114
byte 1 0
align 1
LABELV $969
byte 1 115
byte 1 104
byte 1 97
byte 1 100
byte 1 111
byte 1 119
byte 1 89
byte 1 0
align 1
LABELV $961
byte 1 115
byte 1 104
byte 1 97
byte 1 100
byte 1 111
byte 1 119
byte 1 88
byte 1 0
align 1
LABELV $953
byte 1 102
byte 1 97
byte 1 100
byte 1 101
byte 1 65
byte 1 109
byte 1 111
byte 1 117
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $945
byte 1 102
byte 1 97
byte 1 100
byte 1 101
byte 1 67
byte 1 121
byte 1 99
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $937
byte 1 102
byte 1 97
byte 1 100
byte 1 101
byte 1 67
byte 1 108
byte 1 97
byte 1 109
byte 1 112
byte 1 0
align 1
LABELV $925
byte 1 99
byte 1 117
byte 1 114
byte 1 115
byte 1 111
byte 1 114
byte 1 0
align 1
LABELV $917
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 66
byte 1 117
byte 1 122
byte 1 122
byte 1 83
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 0
align 1
LABELV $909
byte 1 105
byte 1 116
byte 1 101
byte 1 109
byte 1 70
byte 1 111
byte 1 99
byte 1 117
byte 1 115
byte 1 83
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 0
align 1
LABELV $901
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 69
byte 1 120
byte 1 105
byte 1 116
byte 1 83
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 0
align 1
LABELV $893
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 69
byte 1 110
byte 1 116
byte 1 101
byte 1 114
byte 1 83
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 0
align 1
LABELV $885
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 105
byte 1 101
byte 1 110
byte 1 116
byte 1 98
byte 1 97
byte 1 114
byte 1 0
align 1
LABELV $875
byte 1 98
byte 1 105
byte 1 103
byte 1 68
byte 1 121
byte 1 110
byte 1 70
byte 1 111
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $865
byte 1 115
byte 1 109
byte 1 97
byte 1 108
byte 1 108
byte 1 68
byte 1 121
byte 1 110
byte 1 70
byte 1 111
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $855
byte 1 100
byte 1 121
byte 1 110
byte 1 70
byte 1 111
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $845
byte 1 98
byte 1 105
byte 1 103
byte 1 102
byte 1 111
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $835
byte 1 115
byte 1 109
byte 1 97
byte 1 108
byte 1 108
byte 1 70
byte 1 111
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $825
byte 1 102
byte 1 111
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $821
byte 1 125
byte 1 0
align 1
LABELV $812
byte 1 123
byte 1 0
align 1
LABELV $805
byte 1 94
byte 1 49
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 32
byte 1 102
byte 1 105
byte 1 108
byte 1 101
byte 1 32
byte 1 116
byte 1 111
byte 1 111
byte 1 32
byte 1 108
byte 1 97
byte 1 114
byte 1 103
byte 1 101
byte 1 58
byte 1 32
byte 1 37
byte 1 115
byte 1 32
byte 1 105
byte 1 115
byte 1 32
byte 1 37
byte 1 105
byte 1 44
byte 1 32
byte 1 109
byte 1 97
byte 1 120
byte 1 32
byte 1 97
byte 1 108
byte 1 108
byte 1 111
byte 1 119
byte 1 101
byte 1 100
byte 1 32
byte 1 105
byte 1 115
byte 1 32
byte 1 37
byte 1 105
byte 1 0
align 1
LABELV $802
byte 1 94
byte 1 49
byte 1 109
byte 1 101
byte 1 110
byte 1 117
byte 1 32
byte 1 102
byte 1 105
byte 1 108
byte 1 101
byte 1 32
byte 1 110
byte 1 111
byte 1 116
byte 1 32
byte 1 102
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 58
byte 1 32
byte 1 37
byte 1 115
byte 1 44
byte 1 32
byte 1 117
byte 1 115
byte 1 105
byte 1 110
byte 1 103
byte 1 32
byte 1 100
byte 1 101
byte 1 102
byte 1 97
byte 1 117
byte 1 108
byte 1 116
byte 1 10
byte 1 0
align 1
LABELV $782
byte 1 67
byte 1 71
byte 1 95
byte 1 67
byte 1 111
byte 1 110
byte 1 102
byte 1 105
byte 1 103
byte 1 83
byte 1 116
byte 1 114
byte 1 105
byte 1 110
byte 1 103
byte 1 58
byte 1 32
byte 1 98
byte 1 97
byte 1 100
byte 1 32
byte 1 105
byte 1 110
byte 1 100
byte 1 101
byte 1 120
byte 1 58
byte 1 32
byte 1 37
byte 1 105
byte 1 0
align 1
LABELV $770
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 47
byte 1 98
byte 1 97
byte 1 116
byte 1 116
byte 1 112
byte 1 97
byte 1 99
byte 1 107
byte 1 46
byte 1 109
byte 1 100
byte 1 51
byte 1 0
align 1
LABELV $767
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 47
byte 1 106
byte 1 101
byte 1 116
byte 1 112
byte 1 97
byte 1 99
byte 1 107
byte 1 95
byte 1 102
byte 1 108
byte 1 97
byte 1 115
byte 1 104
byte 1 46
byte 1 109
byte 1 100
byte 1 51
byte 1 0
align 1
LABELV $764
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 47
byte 1 106
byte 1 101
byte 1 116
byte 1 112
byte 1 97
byte 1 99
byte 1 107
byte 1 46
byte 1 109
byte 1 100
byte 1 51
byte 1 0
align 1
LABELV $761
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 47
byte 1 117
byte 1 112
byte 1 112
byte 1 101
byte 1 114
byte 1 95
byte 1 108
byte 1 105
byte 1 103
byte 1 104
byte 1 116
byte 1 46
byte 1 115
byte 1 107
byte 1 105
byte 1 110
byte 1 0
align 1
LABELV $758
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 47
byte 1 108
byte 1 111
byte 1 119
byte 1 101
byte 1 114
byte 1 95
byte 1 108
byte 1 105
byte 1 103
byte 1 104
byte 1 116
byte 1 46
byte 1 115
byte 1 107
byte 1 105
byte 1 110
byte 1 0
align 1
LABELV $755
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 98
byte 1 97
byte 1 115
byte 1 101
byte 1 47
byte 1 104
byte 1 101
byte 1 97
byte 1 100
byte 1 95
byte 1 108
byte 1 105
byte 1 103
byte 1 104
byte 1 116
byte 1 46
byte 1 115
byte 1 107
byte 1 105
byte 1 110
byte 1 0
align 1
LABELV $743
byte 1 94
byte 1 55
byte 1 37
byte 1 115
byte 1 32
byte 1 32
byte 1 32
byte 1 32
byte 1 32
byte 1 0
align 1
LABELV $701
byte 1 42
byte 1 37
byte 1 105
byte 1 0
align 1
LABELV $693
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 100
byte 1 111
byte 1 109
byte 1 105
byte 1 110
byte 1 97
byte 1 116
byte 1 105
byte 1 111
byte 1 110
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 115
byte 1 116
byte 1 97
byte 1 116
byte 1 46
byte 1 99
byte 1 102
byte 1 103
byte 1 0
align 1
LABELV $691
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 115
byte 1 116
byte 1 97
byte 1 116
byte 1 46
byte 1 99
byte 1 102
byte 1 103
byte 1 0
align 1
LABELV $689
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 115
byte 1 116
byte 1 97
byte 1 116
byte 1 46
byte 1 99
byte 1 102
byte 1 103
byte 1 0
align 1
LABELV $688
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $685
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $682
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 66
byte 1 108
byte 1 101
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $679
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 66
byte 1 108
byte 1 101
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $676
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 68
byte 1 101
byte 1 115
byte 1 116
byte 1 114
byte 1 111
byte 1 121
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $673
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 68
byte 1 101
byte 1 115
byte 1 116
byte 1 114
byte 1 111
byte 1 121
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $670
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 68
byte 1 97
byte 1 109
byte 1 97
byte 1 103
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $667
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 68
byte 1 97
byte 1 109
byte 1 97
byte 1 103
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $664
byte 1 106
byte 1 101
byte 1 116
byte 1 80
byte 1 97
byte 1 99
byte 1 107
byte 1 65
byte 1 115
byte 1 99
byte 1 101
byte 1 110
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $661
byte 1 106
byte 1 101
byte 1 116
byte 1 80
byte 1 97
byte 1 99
byte 1 107
byte 1 72
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $658
byte 1 106
byte 1 101
byte 1 116
byte 1 80
byte 1 97
byte 1 99
byte 1 107
byte 1 68
byte 1 101
byte 1 115
byte 1 99
byte 1 101
byte 1 110
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $655
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 65
byte 1 99
byte 1 105
byte 1 100
byte 1 84
byte 1 117
byte 1 98
byte 1 101
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $652
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 69
byte 1 118
byte 1 111
byte 1 108
byte 1 118
byte 1 101
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $649
byte 1 112
byte 1 111
byte 1 105
byte 1 115
byte 1 111
byte 1 110
byte 1 67
byte 1 108
byte 1 111
byte 1 117
byte 1 100
byte 1 101
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $646
byte 1 102
byte 1 105
byte 1 114
byte 1 115
byte 1 116
byte 1 80
byte 1 101
byte 1 114
byte 1 115
byte 1 111
byte 1 110
byte 1 80
byte 1 111
byte 1 105
byte 1 115
byte 1 111
byte 1 110
byte 1 67
byte 1 108
byte 1 111
byte 1 117
byte 1 100
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $643
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 97
byte 1 114
byte 1 107
byte 1 115
byte 1 47
byte 1 119
byte 1 97
byte 1 107
byte 1 101
byte 1 0
align 1
LABELV $640
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 97
byte 1 114
byte 1 107
byte 1 115
byte 1 47
byte 1 115
byte 1 104
byte 1 97
byte 1 100
byte 1 111
byte 1 119
byte 1 0
align 1
LABELV $637
byte 1 100
byte 1 105
byte 1 115
byte 1 99
byte 1 111
byte 1 110
byte 1 110
byte 1 101
byte 1 99
byte 1 116
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $634
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 115
byte 1 112
byte 1 114
byte 1 105
byte 1 116
byte 1 101
byte 1 115
byte 1 47
byte 1 99
byte 1 104
byte 1 97
byte 1 116
byte 1 98
byte 1 97
byte 1 108
byte 1 108
byte 1 111
byte 1 111
byte 1 110
byte 1 0
align 1
LABELV $631
byte 1 105
byte 1 99
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 105
byte 1 99
byte 1 111
byte 1 110
byte 1 97
byte 1 95
byte 1 117
byte 1 112
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 101
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $628
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 95
byte 1 112
byte 1 111
byte 1 105
byte 1 115
byte 1 111
byte 1 110
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $625
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 95
byte 1 109
byte 1 101
byte 1 100
byte 1 107
byte 1 105
byte 1 116
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $622
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 51
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $619
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 50
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $616
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 46
byte 1 116
byte 1 103
byte 1 97
byte 1 0
align 1
LABELV $607
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 105
byte 1 99
byte 1 111
byte 1 110
byte 1 95
byte 1 37
byte 1 99
byte 1 0
align 1
LABELV $600
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 104
byte 1 0
align 1
LABELV $597
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 98
byte 1 97
byte 1 114
byte 1 95
byte 1 97
byte 1 0
align 1
LABELV $594
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 98
byte 1 97
byte 1 114
byte 1 0
align 1
LABELV $591
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 0
align 1
LABELV $588
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 0
align 1
LABELV $585
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 112
byte 1 95
byte 1 110
byte 1 111
byte 1 110
byte 1 101
byte 1 0
align 1
LABELV $582
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 116
byte 1 101
byte 1 108
byte 1 101
byte 1 110
byte 1 111
byte 1 100
byte 1 101
byte 1 47
byte 1 114
byte 1 101
byte 1 112
byte 1 95
byte 1 99
byte 1 121
byte 1 108
byte 1 0
align 1
LABELV $579
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 114
byte 1 101
byte 1 100
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 0
align 1
LABELV $576
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 103
byte 1 114
byte 1 101
byte 1 101
byte 1 110
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 0
align 1
LABELV $573
byte 1 99
byte 1 111
byte 1 110
byte 1 115
byte 1 111
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $570
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 116
byte 1 114
byte 1 97
byte 1 99
byte 1 101
byte 1 114
byte 1 0
align 1
LABELV $567
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 116
byte 1 101
byte 1 97
byte 1 109
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 108
byte 1 97
byte 1 121
byte 1 0
align 1
LABELV $564
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 115
byte 1 116
byte 1 97
byte 1 108
byte 1 107
byte 1 0
align 1
LABELV $561
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 98
byte 1 108
byte 1 105
byte 1 112
byte 1 0
align 1
LABELV $558
byte 1 99
byte 1 114
byte 1 101
byte 1 101
byte 1 112
byte 1 0
align 1
LABELV $555
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 101
byte 1 116
byte 1 0
align 1
LABELV $552
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 100
byte 1 97
byte 1 109
byte 1 97
byte 1 103
byte 1 101
byte 1 47
byte 1 102
byte 1 117
byte 1 108
byte 1 108
byte 1 115
byte 1 99
byte 1 114
byte 1 101
byte 1 101
byte 1 110
byte 1 95
byte 1 112
byte 1 97
byte 1 105
byte 1 110
byte 1 98
byte 1 108
byte 1 101
byte 1 110
byte 1 100
byte 1 0
align 1
LABELV $540
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 49
byte 1 50
byte 1 95
byte 1 48
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $539
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 49
byte 1 48
byte 1 95
byte 1 53
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $538
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 57
byte 1 95
byte 1 48
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $537
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 55
byte 1 95
byte 1 53
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $536
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 54
byte 1 95
byte 1 48
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $535
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 52
byte 1 95
byte 1 53
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $534
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 51
byte 1 95
byte 1 48
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $533
byte 1 117
byte 1 105
byte 1 47
byte 1 97
byte 1 115
byte 1 115
byte 1 101
byte 1 116
byte 1 115
byte 1 47
byte 1 110
byte 1 101
byte 1 117
byte 1 116
byte 1 114
byte 1 97
byte 1 108
byte 1 47
byte 1 49
byte 1 95
byte 1 53
byte 1 112
byte 1 105
byte 1 101
byte 1 0
align 1
LABELV $531
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 109
byte 1 105
byte 1 110
byte 1 117
byte 1 115
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $530
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 110
byte 1 105
byte 1 110
byte 1 101
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $529
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 101
byte 1 105
byte 1 103
byte 1 104
byte 1 116
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $528
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 115
byte 1 101
byte 1 118
byte 1 101
byte 1 110
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $527
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 115
byte 1 105
byte 1 120
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $526
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 102
byte 1 105
byte 1 118
byte 1 101
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $525
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 102
byte 1 111
byte 1 117
byte 1 114
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $524
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 116
byte 1 104
byte 1 114
byte 1 101
byte 1 101
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $523
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 116
byte 1 119
byte 1 111
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $522
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 111
byte 1 110
byte 1 101
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $521
byte 1 103
byte 1 102
byte 1 120
byte 1 47
byte 1 50
byte 1 100
byte 1 47
byte 1 110
byte 1 117
byte 1 109
byte 1 98
byte 1 101
byte 1 114
byte 1 115
byte 1 47
byte 1 122
byte 1 101
byte 1 114
byte 1 111
byte 1 95
byte 1 51
byte 1 50
byte 1 98
byte 1 0
align 1
LABELV $518
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 119
byte 1 101
byte 1 97
byte 1 112
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 108
byte 1 99
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 110
byte 1 47
byte 1 119
byte 1 97
byte 1 114
byte 1 110
byte 1 105
byte 1 110
byte 1 103
byte 1 50
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $515
byte 1 109
byte 1 111
byte 1 100
byte 1 101
byte 1 108
byte 1 115
byte 1 47
byte 1 119
byte 1 101
byte 1 97
byte 1 112
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 108
byte 1 99
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 110
byte 1 47
byte 1 119
byte 1 97
byte 1 114
byte 1 110
byte 1 105
byte 1 110
byte 1 103
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $512
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 47
byte 1 114
byte 1 101
byte 1 112
byte 1 97
byte 1 105
byte 1 114
byte 1 101
byte 1 100
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $509
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 47
byte 1 114
byte 1 101
byte 1 112
byte 1 97
byte 1 105
byte 1 114
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $506
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 114
byte 1 101
byte 1 112
byte 1 101
byte 1 97
byte 1 116
byte 1 101
byte 1 114
byte 1 47
byte 1 117
byte 1 115
byte 1 101
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $503
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 104
byte 1 97
byte 1 114
byte 1 100
byte 1 95
byte 1 98
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 50
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $500
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 104
byte 1 97
byte 1 114
byte 1 100
byte 1 95
byte 1 98
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 49
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $497
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 47
byte 1 100
byte 1 97
byte 1 109
byte 1 97
byte 1 103
byte 1 101
byte 1 37
byte 1 100
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $490
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 47
byte 1 112
byte 1 114
byte 1 101
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $487
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 47
byte 1 101
byte 1 120
byte 1 112
byte 1 108
byte 1 111
byte 1 115
byte 1 105
byte 1 111
byte 1 110
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $484
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 47
byte 1 112
byte 1 114
byte 1 101
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $481
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 47
byte 1 100
byte 1 97
byte 1 109
byte 1 97
byte 1 103
byte 1 101
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $478
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 47
byte 1 101
byte 1 120
byte 1 112
byte 1 108
byte 1 111
byte 1 115
byte 1 105
byte 1 111
byte 1 110
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $475
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 101
byte 1 118
byte 1 111
byte 1 108
byte 1 118
byte 1 101
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $472
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 117
byte 1 112
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 101
byte 1 115
byte 1 47
byte 1 109
byte 1 101
byte 1 100
byte 1 107
byte 1 105
byte 1 116
byte 1 47
byte 1 109
byte 1 101
byte 1 100
byte 1 107
byte 1 105
byte 1 116
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $469
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 117
byte 1 112
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 101
byte 1 115
byte 1 47
byte 1 106
byte 1 101
byte 1 116
byte 1 112
byte 1 97
byte 1 99
byte 1 107
byte 1 47
byte 1 104
byte 1 105
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $466
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 117
byte 1 112
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 101
byte 1 115
byte 1 47
byte 1 106
byte 1 101
byte 1 116
byte 1 112
byte 1 97
byte 1 99
byte 1 107
byte 1 47
byte 1 105
byte 1 100
byte 1 108
byte 1 101
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $463
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 117
byte 1 112
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 101
byte 1 115
byte 1 47
byte 1 106
byte 1 101
byte 1 116
byte 1 112
byte 1 97
byte 1 99
byte 1 107
byte 1 47
byte 1 108
byte 1 111
byte 1 119
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $448
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 102
byte 1 111
byte 1 111
byte 1 116
byte 1 115
byte 1 116
byte 1 101
byte 1 112
byte 1 115
byte 1 47
byte 1 99
byte 1 108
byte 1 97
byte 1 110
byte 1 107
byte 1 37
byte 1 105
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $444
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 102
byte 1 111
byte 1 111
byte 1 116
byte 1 115
byte 1 116
byte 1 101
byte 1 112
byte 1 115
byte 1 47
byte 1 115
byte 1 112
byte 1 108
byte 1 97
byte 1 115
byte 1 104
byte 1 37
byte 1 105
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $440
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 102
byte 1 111
byte 1 111
byte 1 116
byte 1 115
byte 1 116
byte 1 101
byte 1 112
byte 1 115
byte 1 47
byte 1 102
byte 1 108
byte 1 101
byte 1 115
byte 1 104
byte 1 37
byte 1 105
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $437
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 102
byte 1 111
byte 1 111
byte 1 116
byte 1 115
byte 1 116
byte 1 101
byte 1 112
byte 1 115
byte 1 47
byte 1 115
byte 1 116
byte 1 101
byte 1 112
byte 1 37
byte 1 105
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $432
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 100
byte 1 105
byte 1 115
byte 1 99
byte 1 111
byte 1 110
byte 1 110
byte 1 101
byte 1 99
byte 1 116
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $429
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 119
byte 1 97
byte 1 116
byte 1 114
byte 1 95
byte 1 117
byte 1 110
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $426
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 119
byte 1 97
byte 1 116
byte 1 114
byte 1 95
byte 1 111
byte 1 117
byte 1 116
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $423
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 119
byte 1 97
byte 1 116
byte 1 114
byte 1 95
byte 1 105
byte 1 110
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $420
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 108
byte 1 97
byte 1 110
byte 1 100
byte 1 49
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $417
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 95
byte 1 116
byte 1 97
byte 1 108
byte 1 107
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $414
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 95
byte 1 116
byte 1 97
byte 1 108
byte 1 107
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $411
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 109
byte 1 105
byte 1 115
byte 1 99
byte 1 47
byte 1 116
byte 1 97
byte 1 108
byte 1 107
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $408
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 119
byte 1 101
byte 1 97
byte 1 112
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 99
byte 1 108
byte 1 105
byte 1 99
byte 1 107
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $405
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 98
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 115
byte 1 47
byte 1 109
byte 1 103
byte 1 116
byte 1 117
byte 1 114
byte 1 114
byte 1 101
byte 1 116
byte 1 47
byte 1 115
byte 1 112
byte 1 105
byte 1 110
byte 1 117
byte 1 112
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $402
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 119
byte 1 101
byte 1 97
byte 1 112
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 99
byte 1 104
byte 1 97
byte 1 110
byte 1 103
byte 1 101
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $399
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 119
byte 1 101
byte 1 97
byte 1 112
byte 1 111
byte 1 110
byte 1 115
byte 1 47
byte 1 116
byte 1 114
byte 1 97
byte 1 99
byte 1 101
byte 1 114
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $396
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 108
byte 1 101
byte 1 118
byte 1 101
byte 1 108
byte 1 52
byte 1 47
byte 1 99
byte 1 104
byte 1 97
byte 1 114
byte 1 103
byte 1 101
byte 1 95
byte 1 115
byte 1 116
byte 1 97
byte 1 114
byte 1 116
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $393
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 108
byte 1 101
byte 1 118
byte 1 101
byte 1 108
byte 1 52
byte 1 47
byte 1 99
byte 1 104
byte 1 97
byte 1 114
byte 1 103
byte 1 101
byte 1 95
byte 1 112
byte 1 114
byte 1 101
byte 1 112
byte 1 97
byte 1 114
byte 1 101
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $390
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 47
byte 1 108
byte 1 101
byte 1 118
byte 1 101
byte 1 108
byte 1 49
byte 1 47
byte 1 103
byte 1 114
byte 1 97
byte 1 98
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $387
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 109
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 47
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 109
byte 1 105
byte 1 110
byte 1 100
byte 1 115
byte 1 112
byte 1 97
byte 1 119
byte 1 110
byte 1 115
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $384
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 109
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 47
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 109
byte 1 105
byte 1 110
byte 1 100
byte 1 100
byte 1 121
byte 1 105
byte 1 110
byte 1 103
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $381
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 109
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 47
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 109
byte 1 105
byte 1 110
byte 1 100
byte 1 97
byte 1 116
byte 1 116
byte 1 97
byte 1 99
byte 1 107
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $378
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 109
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 47
byte 1 114
byte 1 101
byte 1 105
byte 1 110
byte 1 102
byte 1 111
byte 1 114
byte 1 99
byte 1 101
byte 1 109
byte 1 101
byte 1 110
byte 1 116
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $375
byte 1 115
byte 1 111
byte 1 117
byte 1 110
byte 1 100
byte 1 47
byte 1 97
byte 1 110
byte 1 110
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 109
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 47
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 109
byte 1 105
byte 1 110
byte 1 100
byte 1 101
byte 1 118
byte 1 111
byte 1 108
byte 1 118
byte 1 101
byte 1 100
byte 1 46
byte 1 119
byte 1 97
byte 1 118
byte 1 0
align 1
LABELV $366
byte 1 37
byte 1 115
byte 1 0
align 1
LABELV $296
byte 1 37
byte 1 100
byte 1 32
byte 1 37
byte 1 100
byte 1 0
align 1
LABELV $295
byte 1 36
byte 1 0
align 1
LABELV $294
byte 1 85
byte 1 37
byte 1 100
byte 1 32
byte 1 0
align 1
LABELV $286
byte 1 87
byte 1 37
byte 1 100
byte 1 32
byte 1 0
align 1
LABELV $273
byte 1 115
byte 1 118
byte 1 95
byte 1 114
byte 1 117
byte 1 110
byte 1 110
byte 1 105
byte 1 110
byte 1 103
byte 1 0
align 1
LABELV $267
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 114
byte 1 97
byte 1 110
byte 1 115
byte 1 108
byte 1 97
byte 1 116
byte 1 101
byte 1 67
byte 1 101
byte 1 110
byte 1 116
byte 1 101
byte 1 114
byte 1 80
byte 1 114
byte 1 105
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $266
byte 1 99
byte 1 103
byte 1 95
byte 1 99
byte 1 104
byte 1 97
byte 1 116
byte 1 84
byte 1 101
byte 1 97
byte 1 109
byte 1 80
byte 1 114
byte 1 101
byte 1 102
byte 1 105
byte 1 120
byte 1 0
align 1
LABELV $265
byte 1 99
byte 1 103
byte 1 95
byte 1 101
byte 1 109
byte 1 111
byte 1 116
byte 1 105
byte 1 99
byte 1 111
byte 1 110
byte 1 115
byte 1 0
align 1
LABELV $264
byte 1 100
byte 1 101
byte 1 102
byte 1 97
byte 1 117
byte 1 108
byte 1 116
byte 1 0
align 1
LABELV $263
byte 1 118
byte 1 111
byte 1 105
byte 1 99
byte 1 101
byte 1 0
align 1
LABELV $262
byte 1 99
byte 1 103
byte 1 95
byte 1 110
byte 1 111
byte 1 84
byte 1 97
byte 1 117
byte 1 110
byte 1 116
byte 1 0
align 1
LABELV $261
byte 1 112
byte 1 109
byte 1 111
byte 1 118
byte 1 101
byte 1 95
byte 1 109
byte 1 115
byte 1 101
byte 1 99
byte 1 0
align 1
LABELV $260
byte 1 112
byte 1 109
byte 1 111
byte 1 118
byte 1 101
byte 1 95
byte 1 102
byte 1 105
byte 1 120
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $259
byte 1 99
byte 1 111
byte 1 109
byte 1 95
byte 1 99
byte 1 97
byte 1 109
byte 1 101
byte 1 114
byte 1 97
byte 1 77
byte 1 111
byte 1 100
byte 1 101
byte 1 0
align 1
LABELV $258
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 109
byte 1 111
byte 1 111
byte 1 116
byte 1 104
byte 1 67
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 0
align 1
LABELV $257
byte 1 116
byte 1 105
byte 1 109
byte 1 101
byte 1 115
byte 1 99
byte 1 97
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $256
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 105
byte 1 109
byte 1 101
byte 1 115
byte 1 99
byte 1 97
byte 1 108
byte 1 101
byte 1 70
byte 1 97
byte 1 100
byte 1 101
byte 1 83
byte 1 112
byte 1 101
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $255
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 105
byte 1 109
byte 1 101
byte 1 115
byte 1 99
byte 1 97
byte 1 108
byte 1 101
byte 1 70
byte 1 97
byte 1 100
byte 1 101
byte 1 69
byte 1 110
byte 1 100
byte 1 0
align 1
LABELV $254
byte 1 103
byte 1 95
byte 1 115
byte 1 121
byte 1 110
byte 1 99
byte 1 104
byte 1 114
byte 1 111
byte 1 110
byte 1 111
byte 1 117
byte 1 115
byte 1 67
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 0
align 1
LABELV $253
byte 1 99
byte 1 111
byte 1 109
byte 1 95
byte 1 98
byte 1 108
byte 1 111
byte 1 111
byte 1 100
byte 1 0
align 1
LABELV $252
byte 1 99
byte 1 108
byte 1 95
byte 1 112
byte 1 97
byte 1 117
byte 1 115
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $251
byte 1 99
byte 1 103
byte 1 95
byte 1 112
byte 1 114
byte 1 111
byte 1 106
byte 1 101
byte 1 99
byte 1 116
byte 1 105
byte 1 108
byte 1 101
byte 1 78
byte 1 117
byte 1 100
byte 1 103
byte 1 101
byte 1 0
align 1
LABELV $250
byte 1 99
byte 1 103
byte 1 95
byte 1 111
byte 1 112
byte 1 116
byte 1 105
byte 1 109
byte 1 105
byte 1 122
byte 1 101
byte 1 80
byte 1 114
byte 1 101
byte 1 100
byte 1 105
byte 1 99
byte 1 116
byte 1 105
byte 1 111
byte 1 110
byte 1 0
align 1
LABELV $249
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 82
byte 1 97
byte 1 110
byte 1 100
byte 1 111
byte 1 109
byte 1 0
align 1
LABELV $248
byte 1 117
byte 1 105
byte 1 95
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 84
byte 1 101
byte 1 97
byte 1 109
byte 1 86
byte 1 111
byte 1 116
byte 1 101
byte 1 65
byte 1 99
byte 1 116
byte 1 105
byte 1 118
byte 1 101
byte 1 0
align 1
LABELV $247
byte 1 117
byte 1 105
byte 1 95
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 84
byte 1 101
byte 1 97
byte 1 109
byte 1 86
byte 1 111
byte 1 116
byte 1 101
byte 1 65
byte 1 99
byte 1 116
byte 1 105
byte 1 118
byte 1 101
byte 1 0
align 1
LABELV $246
byte 1 117
byte 1 105
byte 1 95
byte 1 118
byte 1 111
byte 1 116
byte 1 101
byte 1 65
byte 1 99
byte 1 116
byte 1 105
byte 1 118
byte 1 101
byte 1 0
align 1
LABELV $245
byte 1 84
byte 1 101
byte 1 120
byte 1 116
byte 1 32
byte 1 110
byte 1 111
byte 1 116
byte 1 32
byte 1 115
byte 1 101
byte 1 116
byte 1 0
align 1
LABELV $244
byte 1 117
byte 1 105
byte 1 95
byte 1 100
byte 1 105
byte 1 97
byte 1 108
byte 1 111
byte 1 103
byte 1 0
align 1
LABELV $243
byte 1 48
byte 1 32
byte 1 48
byte 1 0
align 1
LABELV $242
byte 1 117
byte 1 105
byte 1 95
byte 1 115
byte 1 116
byte 1 97
byte 1 103
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $241
byte 1 117
byte 1 105
byte 1 95
byte 1 99
byte 1 97
byte 1 114
byte 1 114
byte 1 105
byte 1 97
byte 1 103
byte 1 101
byte 1 0
align 1
LABELV $240
byte 1 117
byte 1 105
byte 1 95
byte 1 99
byte 1 117
byte 1 114
byte 1 114
byte 1 101
byte 1 110
byte 1 116
byte 1 67
byte 1 108
byte 1 97
byte 1 115
byte 1 115
byte 1 0
align 1
LABELV $239
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 86
byte 1 111
byte 1 105
byte 1 99
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $238
byte 1 48
byte 1 46
byte 1 54
byte 1 53
byte 1 0
align 1
LABELV $237
byte 1 99
byte 1 103
byte 1 95
byte 1 112
byte 1 97
byte 1 105
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 110
byte 1 100
byte 1 90
byte 1 111
byte 1 111
byte 1 109
byte 1 0
align 1
LABELV $236
byte 1 55
byte 1 46
byte 1 48
byte 1 0
align 1
LABELV $235
byte 1 99
byte 1 103
byte 1 95
byte 1 112
byte 1 97
byte 1 105
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 110
byte 1 100
byte 1 83
byte 1 99
byte 1 97
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $234
byte 1 48
byte 1 46
byte 1 55
byte 1 0
align 1
LABELV $233
byte 1 99
byte 1 103
byte 1 95
byte 1 112
byte 1 97
byte 1 105
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 110
byte 1 100
byte 1 77
byte 1 97
byte 1 120
byte 1 0
align 1
LABELV $232
byte 1 48
byte 1 46
byte 1 53
byte 1 0
align 1
LABELV $231
byte 1 99
byte 1 103
byte 1 95
byte 1 112
byte 1 97
byte 1 105
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 110
byte 1 100
byte 1 68
byte 1 111
byte 1 119
byte 1 110
byte 1 82
byte 1 97
byte 1 116
byte 1 101
byte 1 0
align 1
LABELV $230
byte 1 49
byte 1 48
byte 1 46
byte 1 48
byte 1 0
align 1
LABELV $229
byte 1 99
byte 1 103
byte 1 95
byte 1 112
byte 1 97
byte 1 105
byte 1 110
byte 1 66
byte 1 108
byte 1 101
byte 1 110
byte 1 100
byte 1 85
byte 1 112
byte 1 82
byte 1 97
byte 1 116
byte 1 101
byte 1 0
align 1
LABELV $228
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 112
byte 1 101
byte 1 99
byte 1 116
byte 1 97
byte 1 116
byte 1 111
byte 1 114
byte 1 67
byte 1 111
byte 1 110
byte 1 102
byte 1 105
byte 1 103
byte 1 0
align 1
LABELV $227
byte 1 99
byte 1 103
byte 1 95
byte 1 104
byte 1 117
byte 1 109
byte 1 97
byte 1 110
byte 1 67
byte 1 111
byte 1 110
byte 1 102
byte 1 105
byte 1 103
byte 1 0
align 1
LABELV $226
byte 1 0
align 1
LABELV $225
byte 1 99
byte 1 103
byte 1 95
byte 1 97
byte 1 108
byte 1 105
byte 1 101
byte 1 110
byte 1 67
byte 1 111
byte 1 110
byte 1 102
byte 1 105
byte 1 103
byte 1 0
align 1
LABELV $224
byte 1 117
byte 1 105
byte 1 47
byte 1 104
byte 1 117
byte 1 100
byte 1 46
byte 1 116
byte 1 120
byte 1 116
byte 1 0
align 1
LABELV $223
byte 1 99
byte 1 103
byte 1 95
byte 1 104
byte 1 117
byte 1 100
byte 1 70
byte 1 105
byte 1 108
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $222
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 117
byte 1 116
byte 1 111
byte 1 114
byte 1 105
byte 1 97
byte 1 108
byte 1 0
align 1
LABELV $221
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 105
byte 1 115
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 83
byte 1 99
byte 1 97
byte 1 110
byte 1 110
byte 1 101
byte 1 114
byte 1 80
byte 1 108
byte 1 97
byte 1 110
byte 1 101
byte 1 0
align 1
LABELV $220
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 105
byte 1 115
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 67
byte 1 111
byte 1 109
byte 1 109
byte 1 97
byte 1 110
byte 1 100
byte 1 68
byte 1 105
byte 1 97
byte 1 108
byte 1 111
byte 1 103
byte 1 115
byte 1 0
align 1
LABELV $219
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 105
byte 1 115
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 66
byte 1 117
byte 1 105
byte 1 108
byte 1 100
byte 1 68
byte 1 105
byte 1 97
byte 1 108
byte 1 111
byte 1 103
byte 1 115
byte 1 0
align 1
LABELV $218
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 105
byte 1 115
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 85
byte 1 112
byte 1 103
byte 1 114
byte 1 97
byte 1 100
byte 1 101
byte 1 68
byte 1 105
byte 1 97
byte 1 108
byte 1 111
byte 1 103
byte 1 115
byte 1 0
align 1
LABELV $217
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 105
byte 1 115
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 87
byte 1 97
byte 1 114
byte 1 110
byte 1 105
byte 1 110
byte 1 103
byte 1 68
byte 1 105
byte 1 97
byte 1 108
byte 1 111
byte 1 103
byte 1 115
byte 1 0
align 1
LABELV $216
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 80
byte 1 86
byte 1 83
byte 1 0
align 1
LABELV $215
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 84
byte 1 114
byte 1 97
byte 1 105
byte 1 108
byte 1 115
byte 1 0
align 1
LABELV $214
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 80
byte 1 97
byte 1 114
byte 1 116
byte 1 105
byte 1 99
byte 1 108
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $213
byte 1 99
byte 1 103
byte 1 95
byte 1 108
byte 1 105
byte 1 103
byte 1 104
byte 1 116
byte 1 70
byte 1 108
byte 1 97
byte 1 114
byte 1 101
byte 1 0
align 1
LABELV $212
byte 1 51
byte 1 48
byte 1 48
byte 1 48
byte 1 0
align 1
LABELV $211
byte 1 99
byte 1 103
byte 1 95
byte 1 99
byte 1 111
byte 1 110
byte 1 115
byte 1 111
byte 1 108
byte 1 101
byte 1 76
byte 1 97
byte 1 116
byte 1 101
byte 1 110
byte 1 99
byte 1 121
byte 1 0
align 1
LABELV $210
byte 1 99
byte 1 103
byte 1 95
byte 1 98
byte 1 111
byte 1 117
byte 1 110
byte 1 99
byte 1 101
byte 1 80
byte 1 97
byte 1 114
byte 1 116
byte 1 105
byte 1 99
byte 1 108
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $209
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 112
byte 1 116
byte 1 104
byte 1 83
byte 1 111
byte 1 114
byte 1 116
byte 1 80
byte 1 97
byte 1 114
byte 1 116
byte 1 105
byte 1 99
byte 1 108
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $208
byte 1 54
byte 1 48
byte 1 48
byte 1 0
align 1
LABELV $207
byte 1 99
byte 1 103
byte 1 95
byte 1 102
byte 1 108
byte 1 121
byte 1 83
byte 1 112
byte 1 101
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $206
byte 1 99
byte 1 103
byte 1 95
byte 1 117
byte 1 110
byte 1 108
byte 1 97
byte 1 103
byte 1 103
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $205
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 112
byte 1 114
byte 1 105
byte 1 110
byte 1 116
byte 1 84
byte 1 111
byte 1 103
byte 1 103
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $204
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 116
byte 1 105
byte 1 99
byte 1 107
byte 1 121
byte 1 83
byte 1 112
byte 1 101
byte 1 99
byte 1 0
align 1
LABELV $203
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 105
byte 1 115
byte 1 97
byte 1 98
byte 1 108
byte 1 101
byte 1 66
byte 1 108
byte 1 117
byte 1 101
byte 1 112
byte 1 114
byte 1 105
byte 1 110
byte 1 116
byte 1 69
byte 1 114
byte 1 114
byte 1 111
byte 1 114
byte 1 115
byte 1 0
align 1
LABELV $202
byte 1 99
byte 1 103
byte 1 95
byte 1 119
byte 1 119
byte 1 84
byte 1 111
byte 1 103
byte 1 103
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $201
byte 1 99
byte 1 103
byte 1 95
byte 1 119
byte 1 119
byte 1 70
byte 1 111
byte 1 108
byte 1 108
byte 1 111
byte 1 119
byte 1 0
align 1
LABELV $200
byte 1 51
byte 1 48
byte 1 48
byte 1 0
align 1
LABELV $199
byte 1 99
byte 1 103
byte 1 95
byte 1 119
byte 1 119
byte 1 83
byte 1 109
byte 1 111
byte 1 111
byte 1 116
byte 1 104
byte 1 84
byte 1 105
byte 1 109
byte 1 101
byte 1 0
align 1
LABELV $198
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 66
byte 1 66
byte 1 79
byte 1 88
byte 1 0
align 1
LABELV $197
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 83
byte 1 117
byte 1 114
byte 1 102
byte 1 78
byte 1 111
byte 1 114
byte 1 109
byte 1 97
byte 1 108
byte 1 0
align 1
LABELV $196
byte 1 99
byte 1 103
byte 1 95
byte 1 110
byte 1 111
byte 1 86
byte 1 111
byte 1 105
byte 1 99
byte 1 101
byte 1 84
byte 1 101
byte 1 120
byte 1 116
byte 1 0
align 1
LABELV $195
byte 1 99
byte 1 103
byte 1 95
byte 1 110
byte 1 111
byte 1 86
byte 1 111
byte 1 105
byte 1 99
byte 1 101
byte 1 67
byte 1 104
byte 1 97
byte 1 116
byte 1 115
byte 1 0
align 1
LABELV $194
byte 1 99
byte 1 103
byte 1 95
byte 1 110
byte 1 111
byte 1 80
byte 1 114
byte 1 105
byte 1 110
byte 1 116
byte 1 68
byte 1 117
byte 1 112
byte 1 108
byte 1 105
byte 1 99
byte 1 97
byte 1 116
byte 1 101
byte 1 0
align 1
LABELV $193
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 101
byte 1 97
byte 1 109
byte 1 67
byte 1 104
byte 1 97
byte 1 116
byte 1 115
byte 1 79
byte 1 110
byte 1 108
byte 1 121
byte 1 0
align 1
LABELV $192
byte 1 116
byte 1 101
byte 1 97
byte 1 109
byte 1 111
byte 1 118
byte 1 101
byte 1 114
byte 1 108
byte 1 97
byte 1 121
byte 1 0
align 1
LABELV $191
byte 1 56
byte 1 0
align 1
LABELV $190
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 101
byte 1 97
byte 1 109
byte 1 79
byte 1 118
byte 1 101
byte 1 114
byte 1 108
byte 1 97
byte 1 121
byte 1 77
byte 1 97
byte 1 120
byte 1 80
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 115
byte 1 0
align 1
LABELV $189
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 84
byte 1 101
byte 1 97
byte 1 109
byte 1 79
byte 1 118
byte 1 101
byte 1 114
byte 1 108
byte 1 97
byte 1 121
byte 1 0
align 1
LABELV $188
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 116
byte 1 97
byte 1 116
byte 1 115
byte 1 0
align 1
LABELV $187
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 104
byte 1 105
byte 1 114
byte 1 100
byte 1 80
byte 1 101
byte 1 114
byte 1 115
byte 1 111
byte 1 110
byte 1 83
byte 1 104
byte 1 111
byte 1 117
byte 1 108
byte 1 100
byte 1 101
byte 1 114
byte 1 86
byte 1 105
byte 1 101
byte 1 119
byte 1 77
byte 1 111
byte 1 100
byte 1 101
byte 1 0
align 1
LABELV $186
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 104
byte 1 105
byte 1 114
byte 1 100
byte 1 80
byte 1 101
byte 1 114
byte 1 115
byte 1 111
byte 1 110
byte 1 80
byte 1 105
byte 1 116
byte 1 99
byte 1 104
byte 1 70
byte 1 111
byte 1 108
byte 1 108
byte 1 111
byte 1 119
byte 1 0
align 1
LABELV $185
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 104
byte 1 105
byte 1 114
byte 1 100
byte 1 80
byte 1 101
byte 1 114
byte 1 115
byte 1 111
byte 1 110
byte 1 65
byte 1 110
byte 1 103
byte 1 108
byte 1 101
byte 1 0
align 1
LABELV $184
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 104
byte 1 105
byte 1 114
byte 1 100
byte 1 80
byte 1 101
byte 1 114
byte 1 115
byte 1 111
byte 1 110
byte 1 0
align 1
LABELV $183
byte 1 55
byte 1 53
byte 1 0
align 1
LABELV $182
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 104
byte 1 105
byte 1 114
byte 1 100
byte 1 80
byte 1 101
byte 1 114
byte 1 115
byte 1 111
byte 1 110
byte 1 82
byte 1 97
byte 1 110
byte 1 103
byte 1 101
byte 1 0
align 1
LABELV $181
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 114
byte 1 97
byte 1 99
byte 1 101
byte 1 114
byte 1 108
byte 1 101
byte 1 110
byte 1 103
byte 1 116
byte 1 104
byte 1 0
align 1
LABELV $180
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 114
byte 1 97
byte 1 99
byte 1 101
byte 1 114
byte 1 119
byte 1 105
byte 1 100
byte 1 116
byte 1 104
byte 1 0
align 1
LABELV $179
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 114
byte 1 97
byte 1 99
byte 1 101
byte 1 114
byte 1 99
byte 1 104
byte 1 97
byte 1 110
byte 1 99
byte 1 101
byte 1 0
align 1
LABELV $178
byte 1 99
byte 1 103
byte 1 95
byte 1 102
byte 1 111
byte 1 111
byte 1 116
byte 1 115
byte 1 116
byte 1 101
byte 1 112
byte 1 115
byte 1 0
align 1
LABELV $177
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 104
byte 1 111
byte 1 119
byte 1 109
byte 1 105
byte 1 115
byte 1 115
byte 1 0
align 1
LABELV $176
byte 1 99
byte 1 103
byte 1 95
byte 1 110
byte 1 111
byte 1 112
byte 1 108
byte 1 97
byte 1 121
byte 1 101
byte 1 114
byte 1 97
byte 1 110
byte 1 105
byte 1 109
byte 1 115
byte 1 0
align 1
LABELV $175
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 77
byte 1 111
byte 1 118
byte 1 101
byte 1 0
align 1
LABELV $174
byte 1 99
byte 1 103
byte 1 95
byte 1 110
byte 1 111
byte 1 112
byte 1 114
byte 1 101
byte 1 100
byte 1 105
byte 1 99
byte 1 116
byte 1 0
align 1
LABELV $173
byte 1 99
byte 1 103
byte 1 95
byte 1 101
byte 1 114
byte 1 114
byte 1 111
byte 1 114
byte 1 100
byte 1 101
byte 1 99
byte 1 97
byte 1 121
byte 1 0
align 1
LABELV $172
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 101
byte 1 118
byte 1 101
byte 1 110
byte 1 116
byte 1 115
byte 1 0
align 1
LABELV $171
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 112
byte 1 111
byte 1 115
byte 1 105
byte 1 116
byte 1 105
byte 1 111
byte 1 110
byte 1 0
align 1
LABELV $170
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 101
byte 1 98
byte 1 117
byte 1 103
byte 1 97
byte 1 110
byte 1 105
byte 1 109
byte 1 0
align 1
LABELV $169
byte 1 99
byte 1 103
byte 1 95
byte 1 97
byte 1 110
byte 1 105
byte 1 109
byte 1 115
byte 1 112
byte 1 101
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $168
byte 1 48
byte 1 46
byte 1 51
byte 1 0
align 1
LABELV $167
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 119
byte 1 105
byte 1 110
byte 1 103
byte 1 83
byte 1 112
byte 1 101
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $166
byte 1 48
byte 1 46
byte 1 48
byte 1 48
byte 1 53
byte 1 0
align 1
LABELV $165
byte 1 99
byte 1 103
byte 1 95
byte 1 114
byte 1 117
byte 1 110
byte 1 114
byte 1 111
byte 1 108
byte 1 108
byte 1 0
align 1
LABELV $164
byte 1 48
byte 1 46
byte 1 48
byte 1 48
byte 1 50
byte 1 0
align 1
LABELV $163
byte 1 99
byte 1 103
byte 1 95
byte 1 114
byte 1 117
byte 1 110
byte 1 112
byte 1 105
byte 1 116
byte 1 99
byte 1 104
byte 1 0
align 1
LABELV $162
byte 1 51
byte 1 0
align 1
LABELV $161
byte 1 99
byte 1 103
byte 1 95
byte 1 99
byte 1 101
byte 1 110
byte 1 116
byte 1 101
byte 1 114
byte 1 116
byte 1 105
byte 1 109
byte 1 101
byte 1 0
align 1
LABELV $160
byte 1 99
byte 1 103
byte 1 95
byte 1 103
byte 1 117
byte 1 110
byte 1 90
byte 1 0
align 1
LABELV $159
byte 1 99
byte 1 103
byte 1 95
byte 1 103
byte 1 117
byte 1 110
byte 1 89
byte 1 0
align 1
LABELV $158
byte 1 99
byte 1 103
byte 1 95
byte 1 103
byte 1 117
byte 1 110
byte 1 88
byte 1 0
align 1
LABELV $157
byte 1 50
byte 1 53
byte 1 48
byte 1 0
align 1
LABELV $156
byte 1 99
byte 1 103
byte 1 95
byte 1 116
byte 1 101
byte 1 115
byte 1 108
byte 1 97
byte 1 84
byte 1 114
byte 1 97
byte 1 105
byte 1 108
byte 1 84
byte 1 105
byte 1 109
byte 1 101
byte 1 0
align 1
LABELV $155
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 83
byte 1 112
byte 1 101
byte 1 101
byte 1 100
byte 1 0
align 1
LABELV $154
byte 1 99
byte 1 103
byte 1 95
byte 1 108
byte 1 97
byte 1 103
byte 1 111
byte 1 109
byte 1 101
byte 1 116
byte 1 101
byte 1 114
byte 1 0
align 1
LABELV $153
byte 1 99
byte 1 103
byte 1 95
byte 1 109
byte 1 97
byte 1 114
byte 1 107
byte 1 115
byte 1 0
align 1
LABELV $152
byte 1 99
byte 1 103
byte 1 95
byte 1 99
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 104
byte 1 97
byte 1 105
byte 1 114
byte 1 83
byte 1 105
byte 1 122
byte 1 101
byte 1 0
align 1
LABELV $151
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 67
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 104
byte 1 97
byte 1 105
byte 1 114
byte 1 78
byte 1 97
byte 1 109
byte 1 101
byte 1 115
byte 1 0
align 1
LABELV $150
byte 1 50
byte 1 0
align 1
LABELV $149
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 67
byte 1 114
byte 1 111
byte 1 115
byte 1 115
byte 1 104
byte 1 97
byte 1 105
byte 1 114
byte 1 0
align 1
LABELV $148
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 67
byte 1 104
byte 1 97
byte 1 114
byte 1 103
byte 1 101
byte 1 66
byte 1 97
byte 1 114
byte 1 0
align 1
LABELV $147
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 83
byte 1 110
byte 1 97
byte 1 112
byte 1 115
byte 1 104
byte 1 111
byte 1 116
byte 1 0
align 1
LABELV $146
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 68
byte 1 101
byte 1 109
byte 1 111
byte 1 83
byte 1 116
byte 1 97
byte 1 116
byte 1 101
byte 1 0
align 1
LABELV $145
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 70
byte 1 80
byte 1 83
byte 1 0
align 1
LABELV $144
byte 1 48
byte 1 0
align 1
LABELV $143
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 67
byte 1 108
byte 1 111
byte 1 99
byte 1 107
byte 1 0
align 1
LABELV $142
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 84
byte 1 105
byte 1 109
byte 1 101
byte 1 114
byte 1 0
align 1
LABELV $141
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 50
byte 1 68
byte 1 0
align 1
LABELV $140
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 104
byte 1 97
byte 1 100
byte 1 111
byte 1 119
byte 1 115
byte 1 0
align 1
LABELV $139
byte 1 48
byte 1 46
byte 1 52
byte 1 0
align 1
LABELV $138
byte 1 99
byte 1 103
byte 1 95
byte 1 115
byte 1 116
byte 1 101
byte 1 114
byte 1 101
byte 1 111
byte 1 83
byte 1 101
byte 1 112
byte 1 97
byte 1 114
byte 1 97
byte 1 116
byte 1 105
byte 1 111
byte 1 110
byte 1 0
align 1
LABELV $137
byte 1 49
byte 1 48
byte 1 48
byte 1 0
align 1
LABELV $136
byte 1 99
byte 1 103
byte 1 95
byte 1 118
byte 1 105
byte 1 101
byte 1 119
byte 1 115
byte 1 105
byte 1 122
byte 1 101
byte 1 0
align 1
LABELV $135
byte 1 49
byte 1 0
align 1
LABELV $134
byte 1 99
byte 1 103
byte 1 95
byte 1 100
byte 1 114
byte 1 97
byte 1 119
byte 1 71
byte 1 117
byte 1 110
byte 1 0
align 1
LABELV $131
byte 1 118
byte 1 109
byte 1 77
byte 1 97
byte 1 105
byte 1 110
byte 1 58
byte 1 32
byte 1 117
byte 1 110
byte 1 107
byte 1 110
byte 1 111
byte 1 119
byte 1 110
byte 1 32
byte 1 99
byte 1 111
byte 1 109
byte 1 109
byte 1 97
byte 1 110
byte 1 100
byte 1 32
byte 1 37
byte 1 105
byte 1 0
